...
export function useResourceTags(): UseResourceTagsResult {
  ...
  const remove = useCallback(async (id: string) => {
    try { const resp = await fetch(`${apiBaseUrl}/api/v1/policy/resource-tags/${id}`, { method: 'DELETE', headers: makeHeaders() }); if (!resp.ok) throw new Error(`Delete failed (${resp.status})`); setTags((prev) => prev.filter((t: any) => t.id !== id)); return true; }
    catch (err) { setError(err instanceof Error ? err.message : 'Unknown'); return false; }
  }, [apiBaseUrl, makeHeaders]);
