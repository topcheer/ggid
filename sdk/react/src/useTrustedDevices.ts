...
export function useTrustedDevices(): UseTrustedDevicesResult {
  ...
  const removeTrust = useCallback(async (id: string): Promise<boolean> => {
    try { const resp = await fetch(`${apiBaseUrl}/api/v1/auth/trusted-devices/${id}`, { method: 'DELETE', headers: makeHeaders() }); if (!resp.ok) throw new Error(`Remove failed (${resp.status})`); setDevices((prev) => prev.filter((d: any) => d.id !== id)); return true; }
