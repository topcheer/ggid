...
export default function ZTNAProvidersPage() {
  ...
  const generateTF = (): string => {
    ...
    const entries = Object.entries(prov.config || {}).map(([k, v]: any[]) => `  ${k} = "${v.includes("***") ? "REDACTED" : v}"`).join("\n");
              ...
              <div className="overflow-x-auto"><table className="w-full text-sm">
                ...
                <tbody className="divide-y dark:divide-gray-800">{devices.map(d => (
                  <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30">
                    ...
                    <td className="px-3 py-2"><div className="flex items-center justify-center gap-1">{[["E", d.compliance.disk_encrypted], ["F", d.compliance.firewall], ["A", d.compliance.av_installed], ["U", d.compliance.os_updated]].map(([label, ok]: any[]) => <span key={label as string} className={"h-5 w-5 rounded text-xs font-bold flex items-center justify-center " + (ok ? "bg-green-100 text-green-600 dark:bg-green-900/30" : "bg-red-100 text-red-600 dark:bg-red-900/30")} title={label as string}>{ok ? <Check className="h-3 w-3" /> : <X className="h-3 w-3" />}</span>)}</div></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowRegister(false)}>
          <div role="dialog" aria-modal="true" className="mx-4 w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-4 space-y-3">
              <div><label className="text-sm font-medium">Provider Type</label><div className="mt-2 grid grid-cols-2 gap-2">{Object.entries(providerConfig).map(([key, cfg]: any[]) => { const PIcon = cfg.icon; return <button key={key} onClick={() => { setNewProviderType(key as Provider["type"]); setNewConfig({}); }} aria-pressed={newProviderType === key} className={"flex items-center gap-2 rounded-lg border-2 p-3 " + (newProviderType === key ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-950/30" : "border-gray-200 dark:border-gray-700")}><PIcon className={"h-4 w-4 " + cfg.color} /><span className="text-xs font-medium">{cfg.name}</span></button>; })}</div></div>
