...
export default function ComplianceAutomationConfigPage() {
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow"><h2 className="text-lg font-semibold mb-4">Remediation Triggers</h2><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="py-2">Condition</th><th scope="col">Action</th></tr></thead><tbody>{form.remediation_triggers.map((r: RemediationTrigger, i: number) => (<tr key={i} className="border-b"><td className="py-2">{r.condition}</td><td className="text-xs">{r.action}</td></tr>))}</tbody></table></div>
      <button onClick={handleSave} disabled={saving} aria-label="Save compliance automation configuration" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">{saving ? "Saving..." : "Save Changes"}</button>
    </div>
  );
}
