...
export default function VPNDetectionPage() {
  ...
  const checkIP = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/v1/auth/vpn-check", { method: "POST", headers: { "Authorization": `Bearer ${localStorage.getItem("ggid_access_token") || ""}`, "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" }, body: JSON.stringify({ ips }) });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <button aria-label="Search" onClick={checkIP} disabled={loading} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2"><Search className="w-4 h-4" /> {loading ? "Checking..." : "Check IPs"}</button>
      </div>
