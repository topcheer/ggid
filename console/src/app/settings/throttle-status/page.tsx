...
export default function ThrottleStatusPage() {
  ...
  const fetchData = useCallback(async (user: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/v1/auth/throttle-status?user=${encodeURIComponent(user)}`, { headers: { "Authorization": `Bearer ${localStorage.getItem("ggid_access_token") || ""}`, "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
