...
export default function RiskPostureDashboardPage() {
  ...
  const fetchData = useCallback(async () => { setLoading(true); try { const res = await fetch("/api/v1/audit/risk-posture", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); if (res.ok) setData(await res.json()); } catch { /* noop */ } finally { setLoading(false); } }, []);
  useEffect(() => { fetchData(); }, [fetchData]);
  const scoreColor = data ? (data.overall_score >= 75 ? "#10b981" : data.overall_score >= 50 ? "#f59e0b" : "#ef4444") : "#3b82f6";
  const maxTrend = Math.max(...(data?.trend_30d.map((d: any) => d.score) || [100]), 1);
  return (
    <div className="space-y-6">
      ...
      {data && (<>
        ...
        <div className="overflow-x-auto rounded-lg border dark:border-gray-800"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Finding</th><th className="px-4 py-3 text-left font-medium">Severity</th><th className="px-4 py-3 text-left font-medium">Age</th><th className="px-4 py-3 text-left font-medium">Owner</th><th className="px-4 py-3 text-left font-medium">Status</th></tr></thead><tbody className="divide-y dark:divide-gray-800">{data.open_findings.map((f: any) => (<tr key={f.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3">{f.finding}</td><td className="px-4 py-3"><span className={"px-2 py-0.5 rounded text-xs " + sevColors[f.severity]}>{f.severity}</span></td><td className="px-4 py-3 text-xs">{f.age_days}d</td><td className="px-4 py-3 text-xs font-mono">{f.owner}</td><td className="px-4 py-3"><span className={"text-xs " + (f.status === "open" ? "text-red-600" : "text-green-600")}>{f.status}</span></td></tr>))}</tbody></table></div>
      </>)}
