...
export default function DirectorySnapshotPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/identity/directory-snapshot", { headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><Camera className="w-6 h-6 text-cyan-500" /> Directory Snapshot</h1>
        <p className="text-sm text-gray-500 mt-1">Point-in-time view of user directory with 24h change summary.</p>
      </div>

      {snapshot && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-lg border p-4 dark:border-gray-800 flex items-center gap-3"><Users className="w-8 h-8 text-blue-500" /><div><span className="text-sm text-gray-500">Total Users</span><p className="text-xl font-bold">{snapshot.total_users}</p></div></div>
            <div className="rounded-lg border p-4 dark:border-gray-800 flex items-center gap-3"><TrendingUp className="w-8 h-8 text-green-500" /><div><span className="text-sm text-gray-500">Created (24h)</span><p className="text-xl font-bold text-green-600">{snapshot.changes_24h.created}</p></div></div>
            <div className="rounded-lg border p-4 dark:border-gray-800 flex items-center gap-3"><TrendingDown className="w-8 h-8 text-red-500" /><div><span className="text-sm text-gray-500">Deleted (24h)</span><p className="text-xl font-bold text-red-600">{snapshot.changes_24h.deleted}</p></div></div>
            <div className="rounded-lg border p-4 dark:border-gray-800 flex items-center gap-3"><span className={`text-xl font-bold ${snapshot.changes_24h.net >= 0 ? "text-green-600" : "text-red-600"}`}>{snapshot.changes_24h.net >= 0 ? "+" : ""}{snapshot.changes_24h.net}</span><div><span className="text-sm text-gray-500">Net Change</span><p className="text-xs text-gray-400">{snapshot.changes_24h.modified} modified</p></div></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">By Status</h3>
            ...
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">By Organization</h3>
          ...
          <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
            <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Role</th><th className="px-4 py-3 text-left font-medium">Count</th><th className="px-4 py-3 text-left font-medium">Share</th></tr></thead>
          ...
          <p className="text-xs text-gray-400 text-right">Snapshot taken: {snapshot.snapshot_at}</p>
        </>
      )}
      {!snapshot && !loading && <p className="text-sm text-gray-500 text-center py-8">Loading...</p>}
    </div>
  );
}
