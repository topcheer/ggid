...
interface LocalUserProvisioningPipelineConfig {
  pipeline_stages: LocalPipelineStage[];
  webhook_notifications: { enabled: boolean; url: string };
  error_retry_policy: { max_attempts: number; backoff_seconds: number; dead_letter_queue: boolean };
...
export default function UserProvisioningPipelineConfigPage() {
  ...
  useEffect(() => { if (config) setForm(config as unknown as LocalUserProvisioningPipelineConfig); }, [config]);
