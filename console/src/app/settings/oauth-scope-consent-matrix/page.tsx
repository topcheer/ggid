...
export default function OAuthScopeConsentMatrixPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">{Object.entries(summary).map(([level, count]: any[]) => (<div key={level} className="rounded-lg border p-4 dark:border-gray-800"><div className="flex items-center gap-2"><span className={"w-3 h-3 rounded " + levelColors[level]} /><span className="text-sm capitalize text-gray-500">{level}</span></div><p className="text-xl font-bold mt-1">{count}</p></div>))}</div>

      <div className="overflow-x-auto rounded-lg border dark:border-gray-800"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Scope</th><th className="px-4 py-3 text-left font-medium">Risk</th><th className="px-4 py-3 text-left font-medium">Required Consent</th></tr></thead><tbody className="divide-y dark:divide-gray-800">{data.scopes.map((scope: any) => { const level = data.assignments[scope] || "none"; const risk = data.risk_levels[scope] || "low"; return (<tr key={scope} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3 font-mono text-xs font-medium">{scope}</td><td className="px-4 py-3"><span className={"flex items-center gap-1 text-xs font-medium " + riskColors[risk]}><Shield className="w-3 h-3" /> {risk}</span></td><td className="px-4 py-3"><button onClick={() => cycleLevel(scope)} className={"px-3 py-1 rounded text-xs font-medium text-white " + levelColors[level]}>{level}</button></td></tr>); })}</tbody></table></div>

      <div className="flex items-center gap-4 text-xs">{Object.entries(levelColors).map(([level, color]: any[]) => (<div key={level} className="flex items-center gap-1"><span className={"w-4 h-4 rounded " + color} /> <span className="capitalize">{level}</span></div>))}</div>
    </div>
  );
}
