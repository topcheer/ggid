...
export default function ResourceTagsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-gray-900 dark:text-white"><Tag className="h-6 w-6 text-purple-600" /> Resource Tags</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Tag resources with key-value metadata for policy targeting and filtering.</p></div><button onClick={() => setShowAssign(true)} className="flex items-center gap-2 rounded-lg bg-purple-600 px-4 py-2 text-sm font-medium text-white hover:bg-purple-700"><Plus className="h-4 w-4" /> Assign Tag</button></div>
      ...
      {showAssign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowAssign(false)}>
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-bold text-gray-900 dark:text-white">Assign Tag</h3><button onClick={() => setShowAssign(false)}><X className="h-5 w-5 text-gray-400" /></button></div>
