    ...
    <div className="min-h-screen bg-gray-950 text-white p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-gray-900 rounded-xl p-6"><h2 className="text-sm font-semibold mb-3 flex items-center gap-2"><Snowflake className="w-4 h-4 text-cyan-400" /> Change Freeze Windows</h2><div className="space-y-1">{(data?.freeze_windows ?? []).map((f) => <div key={f.name} className="text-xs bg-gray-800 rounded p-2"><span className="font-medium">{f.name}</span> - <span className="text-gray-400">{f.period}</span></div>))}</div></div>
          <div className="bg-gray-900 rounded-xl p-6 space-y-2"><h2 className="text-sm font-semibold mb-3 flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-green-400" /> Controls</h2><label className="flex items-center gap-2 text-sm"><input type="checkbox" defaultChecked={data?.sod_enforced} /> Enforce Segregation of Duties</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" defaultChecked={data?.emergency_bypass_enabled} /> Emergency bypass (requires C-level)</label></div>
        </div>
      </div>
    </div>
  );
