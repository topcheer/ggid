...
export default function ClientAnalyticsPage() {
  ...
  const maxUsage = Math.max(...(data?.token_usage_30d.map((t: any) => t.count) || [1]), 1);
  const maxScope = Math.max(...(data?.scopes_requested.map((s: any) => s.count) || [1]), 1);
  ...
  return (
    <div className="space-y-6">
      ...
      <select aria-label="Selected id" value={selectedId} onChange={(e) => setSelectedId(e.target.value)} className="px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm"><option value="">Select Client</option>{clients.map((c: any) => <option key={c.client_id} value={c.client_id}>{c.client_name}</option>)}</select>
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold flex items-center gap-2 mb-3"><Users className="w-4 h-4 text-gray-400" /> Top Users</h3><div className="space-y-2">{data.top_users.map((u: any) => <div key={u.user_id} className="flex items-center gap-2"><span className="text-sm font-medium flex-1">{u.username}</span><span className="text-xs text-gray-500">{u.last_active}</span><span className="px-2 py-0.5 rounded text-xs bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 font-bold">{u.requests}</span></div>)}</div></div>
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Scopes Heatmap</h3><div className="space-y-1">{data.scopes_requested.map((s: any) => <div key={s.scope} className="flex items-center gap-2"><span className="text-xs font-mono w-32 truncate">{s.scope}</span><div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-4 overflow-hidden"><div className={"h-full rounded-full " + (s.count / maxScope > 0.7 ? "bg-red-500" : s.count / maxScope > 0.4 ? "bg-yellow-500" : "bg-green-500")} style={{ width: (s.count / maxScope) * 100 + "%" }} /></div><span className="text-xs font-bold w-10 text-right">{s.count}</span></div>)}</div></div>
          </div>
        </>
      )}
