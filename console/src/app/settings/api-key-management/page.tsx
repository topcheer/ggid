...
export default function ApiKeyManagementPage() {
  ...
  return (
    <div className="p-8 space-y-6 max-w-5xl">
      ...
      {keys.some((k: any) => k.status === "active" && new Date(k.expires) < new Date(Date.now() + 30 * 86400000)) && (
      ...
      <div className="bg-white rounded-lg p-6 shadow">
        ...
        <table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="py-2">Name</th><th scope="col">Scopes</th><th>Created</th><th>Expires</th><th>Last Used</th><th>Calls Today</th><th>Status</th><th>Actions</th></tr></thead><tbody>
          {keys.map((k: ApiKey, i: number) => (<tr key={i} className="border-b hover:bg-gray-50"><td className="py-2 font-medium">{k.name}</td><td><div className="flex flex-wrap gap-1">{k.scopes.map((s: any) => <span key={s} className="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-mono">{s}</span>)}</div></td><td className="text-xs text-gray-500">{k.created}</td><td className="text-xs text-gray-500">{k.expires}</td><td className="text-xs text-gray-500">{k.last_used}</td><td>{k.calls_today?.toLocaleString() ?? 0}</td><td><span className={`px-2 py-1 rounded text-xs ${k.status === "active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{k.status}</span></td><td><button className="text-xs text-red-600 hover:underline">Revoke</button></td></tr>))}
        </tbody></table>
        )}
      </div>
    </div>
