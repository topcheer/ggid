  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <section className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div><label className="text-sm font-medium">{t("sessionMgmtConfig.maxConcurrent")}</label><input type="number" min={1} max={20} value={maxConcurrent} onChange={e => setMaxConcurrent(parseInt(e.target.value) || 3)} className="w-24 border rounded px-2 py-1 text-sm mt-1" /></div>
      </section>

      <div className="grid grid-cols-2 gap-4">
        <label className="flex items-center justify-between bg-white rounded-lg shadow p-4"><span className="text-sm font-medium">{t("sessionMgmtConfig.sessionFixation")}</span><input type="checkbox" checked={fixationPrevention} onChange={e => setFixationPrevention(e.target.checked)} className="rounded" /></label>
        <div className="bg-white rounded-lg shadow p-4"><label className="text-sm font-medium">{t("sessionMgmtConfig.stepUpAuth")}: {stepUpTimeout}s</label><input type="range" min={60} max={1800} step={60} value={stepUpTimeout} onChange={e => setStepUpTimeout(parseInt(e.target.value))} className="w-full mt-2" /></div>
      </div>
      ...
      <section className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div className="space-y-2">
          <label className="flex items-center justify-between"><span className="text-sm">{t("sessionMgmtConfig.bindToIp")}</span><input type="checkbox" checked={bindIp} onChange={e => setBindIp(e.target.checked)} className="rounded" /></label>
          <label className="flex items-center justify-between"><span className="text-sm">{t("sessionMgmtConfig.bindToDevice")}</span><input type="checkbox" checked={bindDevice} onChange={e => setBindDevice(e.target.checked)} className="rounded" /></label>
          <label className="flex items-center justify-between"><span className="text-sm">{t("sessionMgmtConfig.bindToGeo")}</span><input type="checkbox" checked={bindGeo} onChange={e => setBindGeo(e.target.checked)} className="rounded" /></label>
        </div>
