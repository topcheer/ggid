...
export default function SessionGeoStatsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/audit/session-geo-stats", { headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Geographic distribution of active sessions with risk indicators.</p>
      </div>

      {data && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-lg border p-4 dark:border-gray-800 flex items-center gap-3"><MapPin className="w-8 h-8 text-cyan-500" /><div><span className="text-sm text-gray-500">Unique Locations</span><p className="text-xl font-bold">{data.unique_locations}</p></div></div>
            <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">Countries</span><p className="text-xl font-bold mt-1">{data.countries.length}</p></div>
            <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">Top City Sessions</span><p className="text-xl font-bold mt-1">{data.top_cities[0]?.sessions || 0}</p></div>
            <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">Risk Geographies</span><p className="text-xl font-bold text-red-600 mt-1">{data.risk_geographies.length}</p></div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4">
              ...
              ))}{data.top_cities.length === 0 && <p className="text-xs text-gray-400">No data.</p>}</div>
            </div>

            <div className="rounded-lg border dark:border-gray-800 p-4">
              <h3 className="text-sm font-semibold mb-3">Countries</h3>
          ...
          {data.risk_geographies.length > 0 && (
            <div className="rounded-lg border dark:border-gray-800 p-4">
              <h3 className="text-sm font-semibold flex items-center gap-2 mb-3"><AlertTriangle className="w-4 h-4 text-red-500" /> Risk Geographies</h3>
          ...
          <div className="rounded-lg border dark:border-gray-800 p-4">
            <h3 className="text-sm font-semibold mb-3">Heatmap Grid</h3>
            <div className="grid grid-cols-8 gap-1">
              ...
              {data.heatmap.length === 0 && <p className="text-xs text-gray-400 col-span-8">No heatmap data.</p>}
            </div>
          </div>
        </>
      )}
