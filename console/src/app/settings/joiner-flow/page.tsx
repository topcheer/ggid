...
export default function JoinerFlowPage() {
  ...
  const [form, setForm] = useState<JoinerData>({ employee_id: "", start_date: "", department: "", role_templates: [], provision_apps: availableApps.map((a: any) => ({ id: a, name: a, auto: true })), preboarding: [{ id: "t1", label: "Create AD account", done: false }, { id: "t2", label: "Assign laptop", done: false }, { id: "t3", label: "Provision email", done: false }, { id: "t4", label: "Schedule orientation", done: false }], status: "draft" });
  const [submitted, setSubmitted] = useState(false);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const loadData = useCallback(async () => {
    setLoading(true); setError(null);
    try { const res = await fetch("/api/v1/identity/joiner-flow/templates", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); if (res.ok) { const d = await res.json(); if (d.templates) { /* could populate availableTemplates */ } } }
  ...
  const toggleTemplate = (t: string) => {
    setForm((prev) => ({ ...prev, role_templates: prev.role_templates.includes(t) ? prev.role_templates.filter((x: any) => x !== t) : [...prev.role_templates, t] }));
  };

  const toggleApp = (id: string) => setForm((prev) => ({ ...prev, provision_apps: prev.provision_apps.map((a: any) => a.id === id ? { ...a, auto: !a.auto } : a) }));

  const toggleTask = (id: string) => setForm((prev) => ({ ...prev, preboarding: prev.preboarding.map((t: any) => t.id === id ? { ...t, done: !t.done } : t) }));

  const submit = useCallback(async () => {
    if (!form.employee_id || !form.start_date) return;
    try { await fetch("/api/v1/identity/joiner-flow", { method: "POST", headers: { ...authHeader(), "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" }, body: JSON.stringify(form) }); setSubmitted(true); }
    catch { /* noop */ }
  }, [form]);

  const tasksDone = form.preboarding.filter((t: any) => t.done).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4">
        ...
        <div className="flex flex-wrap gap-2">{availableTemplates.map((t: any) => (
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4">
        ...
        <div className="space-y-1">{form.provision_apps.map((a: any) => (
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4">
        ...
        <div className="space-y-1">{form.preboarding.map((t: any) => (
