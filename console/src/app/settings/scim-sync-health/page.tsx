...
export default function ScimSyncHealthPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/identity/scim-sync-health", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
      ...
      {data && (
        <>
          ...
          {data.provisioning_errors.length > 0 && (
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold flex items-center gap-2 mb-3"><AlertTriangle className="w-4 h-4 text-red-500" /> Provisioning Errors</h3><div className="space-y-1">{data.provisioning_errors.map((e: any, i: number) => (<div key={i} className="flex items-center gap-2 text-sm"><span className="text-xs text-gray-400">{e.timestamp}</span><span className="font-mono text-xs text-gray-500">{e.user_id}</span><span className="text-red-600">{e.error}</span></div>))}</div></div>
          )}
        </>
      )}
