...
export default function TokenRotationPolicyPage() {
  ...
  const updateClient = (id: string, field: string, value: string | number | boolean) => { if (!data) return; setData({ ...data, clients: data.clients.map((c: any) => c.client_id === id ? { ...c, [field]: value } : c) }); };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold flex items-center gap-2"><RefreshCw className="w-6 h-6 text-blue-500" /> {t("tokenRotationPolicy.title")}</h1><p className="text-sm text-gray-500 mt-1">Configure per-client token rotation policies.</p></div><button aria-label="Save" onClick={save} disabled={saving || !data} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium flex items-center gap-2"><Save className="w-4 h-4" /> {saving ? "Saving..." : "Save"}</button></div>
      {data && (<>
