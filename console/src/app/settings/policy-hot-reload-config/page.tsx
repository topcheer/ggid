  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-4">
        ...
        <div className="flex items-center gap-3"><input aria-label="Form" type="checkbox" checked={form.watch_enabled} onChange={(e) => setForm({ ...form, watch_enabled: e.target.checked })} className="w-4 h-4" /><label>Watch Enabled</label></div>
        <div className="flex items-center gap-3"><input aria-label="Form" type="checkbox" checked={form.atomic_swap} onChange={(e) => setForm({ ...form, atomic_swap: e.target.checked })} className="w-4 h-4" /><label>Atomic Swap</label></div>
        <div><label className="block text-sm font-medium mb-1">Cache Invalidation Strategy</label><select aria-label="form" value={form.cache_invalidation_strategy} onChange={(e) => setForm({ ...form, cache_invalidation_strategy: e.target.value as PolicyHotReloadConfig["cache_invalidation_strategy"] })} className="border rounded px-3 py-2"><option value="all">All</option><option value="lazy">Lazy</option><option value="versioned">Versioned</option></select></div>
        <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Version Check Interval (ms)</label><input aria-label="form" type="number" value={form.version_check_interval_ms} onChange={(e) => setForm({ ...form, version_check_interval_ms: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div><div><label className="block text-sm font-medium mb-1">Max Reload Concurrency</label><input type="number" value={form.max_reload_concurrency} onChange={(e) => setForm({ ...form, max_reload_concurrency: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div></div>
        <div className="flex items-center gap-3"><input aria-label="Form" type="checkbox" checked={form.rollback_on_error} onChange={(e) => setForm({ ...form, rollback_on_error: e.target.checked })} className="w-4 h-4" /><label>Rollback on Error</label></div>
      </div>
