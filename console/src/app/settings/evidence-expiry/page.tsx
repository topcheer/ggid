...
export default function EvidenceExpiryPage() {
  ...
  const refreshOne = async (id: string) => {
    ...
    try {
      await fetch(`/api/v1/audit/evidence-expiry/${id}/refresh`, {
        method: "POST",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {(expired.length > 0 || expiring.length > 0) && (
          <button onClick={batchRefresh} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 flex items-center gap-2"><RefreshCw className="w-4 h-4" /> Batch Refresh ({expired.length + expiring.length})</button>
        )}
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-lg border p-4 dark:border-gray-800" onClick={() => setFilterStatus(filterStatus === "valid" ? "all" : "valid") as any}>
          ...
          <tbody className="divide-y dark:divide-gray-800">
            {filtered.map((e) => (
              <tr key={e.id} className={`hover:bg-gray-50 dark:hover:bg-gray-900/30 ${rowBorder[e.status]}`}>
                <td className="px-4 py-3 font-mono text-xs">{e.control_id}</td>
                <td className="px-4 py-3">{e.framework}</td>
                <td className="px-4 py-3 text-gray-500">{e.evidence_type}</td>
                <td className="px-4 py-3 text-gray-500">{e.collected_at}</td>
                <td className="px-4 py-3 text-gray-500">{e.expires_at}</td>
                <td className="px-4 py-3">
                  <span className={`font-bold ${e.days_remaining <= 0 ? "text-red-600" : e.days_remaining <= 7 ? "text-orange-600" : e.days_remaining <= 30 ? "text-yellow-600" : "text-green-600"}`}>{e.days_remaining}</span>
                </td>
                <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded text-xs ${urgencyColor[e.status]}`}>{e.status}</span></td>
                <td className="px-4 py-3">
                  <button onClick={() => refreshOne(e.id)} disabled={refreshing.has(e.id)} className="text-xs font-medium text-blue-600 hover:underline disabled:opacity-50 flex items-center gap-1">
                    <RefreshCw className={`w-3 h-3 ${refreshing.has(e.id) ? "animate-spin" : ""}`} />
                    {refreshing.has(e.id) ? "Refreshing..." : "Refresh"}
                  </button>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && !loading && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500">No evidence items found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
