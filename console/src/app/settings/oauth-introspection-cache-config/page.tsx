  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-4">
        ...
        <div><label className="block text-sm font-medium mb-1">Cache Key Strategy</label><select aria-label="Select option" value={form.cache_key_strategy} onChange={(e) => setForm({ ...form, cache_key_strategy: e.target.value as OAuthIntrospectionCacheConfig["cache_key_strategy"] })} className="border rounded px-3 py-2"><option value="token_hash">Token Hash</option><option value="token_jti">Token JTI</option><option value="client_token">Client + Token</option></select></div>
        <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">TTL (s)</label><input aria-label="form" type="number" value={form.ttl_seconds} onChange={(e) => setForm({ ...form, ttl_seconds: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div><div><label className="block text-sm font-medium mb-1">Max Entries</label><input type="number" value={form.max_entries} onChange={(e) => setForm({ ...form, max_entries: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div></div>
        <div className="flex items-center gap-3"><input aria-label="Form" type="checkbox" checked={form.invalidation_on_revocation} onChange={(e) => setForm({ ...form, invalidation_on_revocation: e.target.checked })} className="w-4 h-4" /><label>Invalidation on Revocation</label></div>
        <div className="flex items-center gap-3"><input aria-label="Form" type="checkbox" checked={form.stampede_prevention} onChange={(e) => setForm({ ...form, stampede_prevention: e.target.checked })} className="w-4 h-4" /><label>Stampede Prevention</label></div>
      </div>
