...
export default function ScimProvisioningPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await fetch("/api/v1/identity/scim-provisioning", { headers: { "Authorization": `Bearer ${localStorage.getItem("ggid_access_token") || ""}`, "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); if (res.ok) setData(await res.json()); }
  ...
  return (
    <div className="space-y-6">
      ...
      {data && (<>
        ...
        <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Provisioning Rules</h3><div className="flex gap-4">{(["create", "update", "deactivate"] as const).map((r) => (<label key={r} className="flex items-center gap-2 cursor-pointer"><input aria-label="Data" type="checkbox" checked={data.rules[r]} readOnly className="rounded" /><span className="text-sm capitalize">{r}</span></label>))}</div></div>
