...
export default function PolicyApprovalWorkflowPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await fetch("/api/v1/policy/approval-workflow", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); if (res.ok) { const d = await res.json(); setApprovals(d.approvals || d || []); } }
    catch { /* noop */ }
  ...
  const decide = async (id: string, decision: "approved" | "rejected") => {
    try { await fetch("/api/v1/policy/approval-workflow/" + id, { method: "POST", headers: { ...authHeader(), "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" }, body: JSON.stringify({ decision, comment }) }); setComment(""); fetchData(); }
    catch { /* noop */ }
  };
          ...
          <div key={a.id} className="rounded-lg border dark:border-gray-800">
            ...
            {expanded === a.id && (
              <div className="border-t dark:border-gray-800 p-3 bg-gray-50 dark:bg-gray-900/30 space-y-3">
                <div><h4 className="text-xs font-semibold text-gray-500 mb-2">Approval Chain</h4><div className="flex items-center gap-2">{a.approval_chain.map((step: any, i: number) => (<div key={i} className="flex items-center gap-1"><span className={"text-xs px-2 py-0.5 rounded " + (step.status === "approved" ? "bg-green-100 dark:bg-green-900/30 dark:text-green-400" : step.status === "rejected" ? "bg-red-100 dark:bg-red-900/30 dark:text-red-400" : "bg-gray-100 dark:bg-gray-800")}>{step.approver}</span>{i < a.approval_chain.length - 1 && <span className="text-gray-300">{"->"}</span>}</div>))}</div></div>
                {a.comments.length > 0 && (<div><h4 className="text-xs font-semibold text-gray-500 mb-1">Comments</h4><div className="space-y-1">{a.comments.map((c: any, i: number) => (<div key={i} className="text-xs text-gray-500"><span className="font-medium">{c.author}:</span> {c.text} <span className="text-gray-400">({c.timestamp})</span></div>))}</div></div>)}
                <div className="flex items-center gap-2"><input aria-label="Add comment..." type="text" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Add comment..." className="flex-1 px-2 py-1 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs" /><button onClick={() => decide(a.id, "approved")} className="px-3 py-1 rounded text-xs bg-green-600 text-white flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Approve</button><button onClick={() => decide(a.id, "rejected")} className="px-3 py-1 rounded text-xs bg-red-600 text-white flex items-center gap-1"><XCircle className="w-3 h-3" /> Reject</button></div>
              </div>
            )}
          </div>
        ))}
