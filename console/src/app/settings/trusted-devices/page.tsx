...
export default function TrustedDevicesPage() {
  ...
  const handleRemove = async (id: string) => { setActioning(id); try { await apiFetch(`/api/v1/auth/trusted-devices/${id}`, { method: "DELETE" }); setDevices((p) => p.filter((d: any) => d.id !== id)); } catch { setError("Remove failed"); } finally { setActioning(null); } };
  const handleToggleMfa = async (d: TrustedDevice) => { setActioning(d.id); try { await apiFetch(`/api/v1/auth/trusted-devices/${d.id}`, { method: "PATCH", body: JSON.stringify({ mfa_bypass_enabled: !d.mfa_bypass_enabled }) }); setDevices((p) => p.map((x: any) => x.id === d.id ? { ...x, mfa_bypass_enabled: !x.mfa_bypass_enabled } : x)); } catch { setError("Toggle failed"); } finally { setActioning(null); } };
