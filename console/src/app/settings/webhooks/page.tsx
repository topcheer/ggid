...
export default function WebhooksPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (<div role="alert" className="flex items-center gap-2 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-400 focus:outline-none focus:ring-2 focus:ring-blue-500"><AlertCircle className="h-4 w-4 shrink-0" />{error}<button onClick={() => setError(null)} aria-label="Dismiss" className="ml-auto"><X className="h-4 w-4" /></button></div>)}
          ...
          {endpoints.length === 0 ? <div className={card}><div className="py-12 text-center"><Webhook className="mx-auto h-12 w-12 text-gray-300" /><p className="mt-4 text-sm text-gray-400">{t("webhooks.noEndpoints")}</p></div></div> : (
            <div className="space-y-2">{endpoints.map(ep => (
              <div key={ep.id} className={`${card} flex items-center justify-between !p-3`}>
                <div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-purple-100 dark:bg-purple-900/30 focus:outline-none focus:ring-2 focus:ring-blue-500"><Webhook className="h-4 w-4 text-purple-500" /></div><div><div className="flex items-center gap-2"><code className="text-xs font-mono truncate max-w-md">{ep.url}</code><span className={`px-1.5 py-0.5 rounded text-xs font-medium ${ep.status === "active" ? "bg-green-100 dark:bg-green-900/30 text-green-600" : "bg-gray-100 dark:bg-gray-800 text-gray-400"}`}>{ep.status}</span></div><div className="flex flex-wrap gap-1 mt-0.5">{(ep.events || []).slice(0, 4).map(ev => <span key={ev} className="px-1 py-0.5 rounded bg-gray-100 dark:bg-gray-700 dark:bg-gray-700 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">{ev}</span>)}{(ep.events || []).length > 4 && <span className="text-xs text-gray-400">+{ep.events.length - 4}</span>}</div><p className="text-xs text-gray-400">{ep.last_delivery ? `${t("webhooks.lastDelivery")}: ${new Date(ep.last_delivery).toLocaleString()}` : t("webhooks.noDeliveries")}</p></div></div>
                <button onClick={() => deleteEndpoint(ep.id)} disabled={actionLoading === `del-${ep.id}`} aria-label="Delete" className="rounded p-1.5 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 focus:outline-none focus:ring-2 focus:ring-blue-500">{actionLoading === `del-${ep.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}</button>
              </div>
            ))}</div>
          )}
        </div>
      ...
      {tab === "catalog" && (
        <div className="space-y-4">{EVENT_CATALOG.map(cat => (
          <div key={cat.category} className={card}>
            <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold"><span className="px-2 py-0.5 rounded bg-purple-100 dark:bg-purple-900/30 text-purple-600 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">{cat.category}.*</span></h3>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowForm(false)}>
          <div role="dialog" aria-modal="true" className="mx-4 w-full max-w-lg rounded-xl bg-white dark:bg-gray-800 p-6 shadow-xl dark:bg-gray-800 max-h-[90vh] overflow-y-auto focus:outline-none focus:ring-2 focus:ring-blue-500" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-4 space-y-3">
              <div><label className="text-sm font-medium">{t("webhooks.url")}</label><input type="text" value={fUrl} onChange={e => setFUrl(e.target.value)} placeholder="https://hooks.example.com/ggid" className="mt-1 w-full rounded-lg border dark:border-gray-700 dark:bg-gray-900 px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500" autoFocus /></div>
              <div><label className="text-sm font-medium">{t("webhooks.subscribeEvents")}</label>
                <div className="mt-1 max-h-48 overflow-y-auto space-y-1 rounded-lg border dark:border-gray-700 p-2">{allEvents.map(ev => <label key={ev} className="flex items-center gap-2 cursor-pointer rounded p-1 hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-800 dark:hover:bg-gray-900/50 focus:outline-none focus:ring-2 focus:ring-blue-500"><input type="checkbox" checked={fEvents.includes(ev)} onChange={() => toggleEvent(ev)} className="rounded" /><code className="text-xs font-mono">{ev}</code></label>)}</div>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2"><button onClick={() => setShowForm(false)} className="rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 text-sm dark:border-gray-700">{t("common.cancel")}</button><button onClick={createEndpoint} disabled={!fUrl || actionLoading === "create"} className="rounded-lg bg-purple-600 px-4 py-2 text-sm font-medium text-white hover:bg-purple-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500">{actionLoading === "create" ? <Loader2 className="h-4 w-4 animate-spin" /> : t("webhooks.create")}</button></div>
          </div>
        </div>
      )}
    </div>
