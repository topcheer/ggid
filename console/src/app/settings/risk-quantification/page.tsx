...
export default function RiskQuantificationPage() {
  ...
  const maxTrend = data ? Math.max(...data.trend_30d.map((t: any) => t.score), 1) : 1;
  ...
  return (
    <div className="space-y-6">
      ...
      {data && (<>
        ...
        <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Risk Factors</h3><div className="space-y-2">{data.factors.map((f: any) => (<div key={f.factor} className="flex items-center gap-3"><span className="text-sm w-40 truncate">{f.factor}</span><div className="flex-1"><div className="flex items-center gap-2"><div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-5"><div className="h-full rounded-full" style={{ width: (f.contribution / 100) * 100 + "%", background: f.contribution > 50 ? "#ef4444" : f.contribution > 25 ? "#f59e0b" : "#10b981" }} /></div><span className="text-xs font-bold w-12 text-right">{f.contribution.toFixed(0)}%</span></div></div></div>))}</div></div>
