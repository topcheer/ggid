...
export default function RiskQuantificationPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/audit/risk-quantification", { headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  return (
    <div className="space-y-6">
      ...
      {data && (<>
        ...
        <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-gray-400" /> 30-Day Trend</h3><div className="flex items-end gap-1 h-24">{data.trend_30d.map((t, i) => (<div key={i} className="flex-1 rounded-t" style={{ height: (t.score / maxTrend) * 100 + "%", background: t.score >= 70 ? "#ef4444" : t.score >= 40 ? "#f59e0b" : "#10b981", minHeight: "2px" }} title={t.date + ": " + t.score} />))}</div></div>
      </>)}
