...
export default function LeaverFlowPage() {
  ...
  const toggleTask = (id: string) => { if (!data) return; setData({ ...data, tasks: data.tasks.map((t: any) => t.id === id ? { ...t, done: !t.done, status: !t.done ? "done" : "pending" } : t), completion_pct: Math.round((data.tasks.filter((t: any) => t.id !== id ? t.done : !t.done).length / data.tasks.length) * 100) }); };
  ...
  return (
    <div className="space-y-6">
      ...
      <select aria-label="Employee id" value={employeeId} onChange={(e) => setEmployeeId(e.target.value)} className="px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm"><option value="">Select Employee</option>{users.map((u: any) => <option key={u.user_id} value={u.user_id}>{u.username}</option>)}</select>
