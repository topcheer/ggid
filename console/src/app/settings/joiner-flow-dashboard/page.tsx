  ...
  return (
    <div className="space-y-6">
      ...
      {data && (<>
        ...
        <div className="space-y-3">{data.pending.map((item: any) => (<div key={item.id} className="rounded-lg border dark:border-gray-800 p-3"><div className="flex items-center justify-between"><div><span className="font-medium text-sm">{item.employee}</span><span className="text-xs text-gray-400 ml-2">{t("big1.joinerFlowDashboard.starts")}{item.start_date}</span></div><div className="flex items-center gap-2"><div className="w-24 bg-gray-100 dark:bg-gray-800 rounded-full h-2 overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: (item.steps_completed / item.total_steps) * 100 + "%" }} /></div><span className="text-xs font-bold">{item.steps_completed}/{item.total_steps}</span></div></div>{item.blocked_items.length > 0 && <div className="mt-2 flex items-center gap-1 text-xs text-orange-600"><AlertTriangle className="w-3 h-3" />{t("big1.joinerFlowDashboard.blocked")}{item.blocked_items.join(", ")}</div>}<div className="mt-2 flex flex-wrap gap-1">{item.provisioning.map((p: any, i: number) => (<span key={i} className={"px-2 py-0.5 rounded text-xs " + (p.status === "done" ? "bg-green-100 dark:bg-green-900/30 dark:text-green-400" : p.status === "failed" ? "bg-red-100 dark:bg-red-900/30 dark:text-red-400" : "bg-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-400")}>{p.app}: {p.status}</span>))}</div></div>))}{data.pending.length === 0 && <p className="text-sm text-gray-500 text-center py-4">{t("big1.joinerFlowDashboard.noPendingOnboarding")}</p>}</div>
        {data.upcoming_starts.length > 0 && <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-2">{t("big1.joinerFlowDashboard.upcomingStarts")}</h3><div className="space-y-1">{data.upcoming_starts.map((u: any, i: number) => (<div key={i} className="flex items-center gap-2 text-sm"><Clock className="w-3.5 h-3.5 text-gray-400" /><span className="font-medium">{u.employee}</span><span className="text-xs text-gray-400">{u.start_date}</span></div>))}</div></div>}
      </>)}
