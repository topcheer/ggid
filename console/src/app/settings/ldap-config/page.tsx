    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <section className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div><label className="text-sm font-medium">Connection Pool Size</label><input type="number" min={1} max={100} value={poolSize} onChange={e => setPoolSize(parseInt(e.target.value) || 10)} className="w-full border rounded px-2 py-1 text-sm mt-1" /></div>
          <div><label className="text-sm font-medium">Sync Interval (s)</label><input type="number" min={60} value={syncInterval} onChange={e => setSyncInterval(parseInt(e.target.value) || 300)} className="w-full border rounded px-2 py-1 text-sm mt-1" /></div>
          <div className="flex items-end"><button onClick={testConnection} disabled={testing} className="px-4 py-2 bg-blue-600 text-white rounded text-sm disabled:opacity-50 w-full">{testing ? 'Testing...' : 'Test Connection'}</button></div>
        </div>
        {testResult && <div className="text-sm p-3 rounded bg-green-50 text-green-700">{testResult}</div>}
      </section>

      <div className="grid grid-cols-2 gap-4">
        <label className="flex items-center justify-between bg-white rounded-lg shadow p-4"><span className="text-sm font-medium">START_TLS</span><input type="checkbox" checked={startTls} onChange={e => setStartTls(e.target.checked)} className="rounded" /></label>
        <label className="flex items-center justify-between bg-white rounded-lg shadow p-4"><span className="text-sm font-medium">Auto-Provision Users</span><input type="checkbox" checked={autoProvision} onChange={e => setAutoProvision(e.target.checked)} className="rounded" /></label>
      </div>
