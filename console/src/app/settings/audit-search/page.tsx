  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4 space-y-3">
        <div className="flex items-center gap-2"><div className="relative flex-1"><Search className="absolute left-2 top-2.5 w-4 h-4 text-gray-400" /><input aria-label="Search audit events..." type="text" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") search(); }} placeholder="Search audit events..." className="w-full pl-8 pr-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" /></div><button onClick={() => search()} disabled={loading} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50">Search</button></div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
          <input aria-label="User" type="text" placeholder="User" value={filters.user} onChange={(e) => setFilters({ ...filters, user: e.target.value })} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs" />
          <input aria-label="Action" type="text" placeholder="Action" value={filters.action} onChange={(e) => setFilters({ ...filters, action: e.target.value })} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs" />
          <input aria-label="Resource" type="text" placeholder="Resource" value={filters.resource} onChange={(e) => setFilters({ ...filters, resource: e.target.value })} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs" />
          <select aria-label="Filter" value={filters.severity} onChange={(e) => setFilters({ ...filters, severity: e.target.value })} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs"><option value="">Severity</option><option value="info">Info</option><option value="warning">Warning</option><option value="error">Error</option><option value="critical">Critical</option></select>
          <input aria-label="Input field" type="date" value={filters.start_date} onChange={(e) => setFilters({ ...filters, start_date: e.target.value })} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs" />
          <input aria-label="Input field" type="date" value={filters.end_date} onChange={(e) => setFilters({ ...filters, end_date: e.target.value })} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs" />
        </div>
