...
export default function NatsJetstreamPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-lg border dark:border-gray-800"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Stream</th><th className="px-4 py-3 text-left font-medium">Subjects</th><th className="px-4 py-3 text-left font-medium">Messages</th><th className="px-4 py-3 text-left font-medium">Size</th><th className="px-4 py-3 text-left font-medium">Consumers</th><th className="px-4 py-3 text-left font-medium">Last Msg</th></tr></thead><tbody className="divide-y dark:divide-gray-800">{data.streams.map((s: any) => (<tr key={s.name} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3 font-mono text-xs font-medium">{s.name}</td><td className="px-4 py-3"><div className="flex flex-wrap gap-1">{s.subjects.slice(0, 3).map((sub: any) => (<span key={sub} className="px-1.5 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-800 font-mono">{sub}</span>))}{s.subjects.length > 3 && <span className="text-xs text-gray-400">+{s.subjects.length - 3}</span>}</div></td><td className="px-4 py-3 font-bold">{s.msgs.toLocaleString()}</td><td className="px-4 py-3 text-xs text-gray-500">{(s.bytes / 1024 / 1024).toFixed(1)} MB</td><td className="px-4 py-3"><span className="px-2 py-0.5 rounded text-xs bg-purple-100 dark:bg-purple-900/30 dark:text-purple-400">{s.consumer_count}</span></td><td className="px-4 py-3 text-xs text-gray-400">{s.last_msg}</td></tr>))}</tbody></table></div>

      {data.consumers.length > 0 && (<div className="overflow-x-auto rounded-lg border dark:border-gray-800"><h3 className="text-sm font-semibold px-4 py-3 border-b dark:border-gray-800">Consumers</h3><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Name</th><th className="px-4 py-3 text-left font-medium">Stream</th><th className="px-4 py-3 text-left font-medium">Delivered</th><th className="px-4 py-3 text-left font-medium">Pending</th><th className="px-4 py-3 text-left font-medium">Status</th></tr></thead><tbody className="divide-y dark:divide-gray-800">{data.consumers.map((c: any, i: number) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3 font-mono text-xs">{c.name}</td><td className="px-4 py-3 text-xs text-gray-500">{c.stream}</td><td className="px-4 py-3 font-bold">{c.delivered.toLocaleString()}</td><td className="px-4 py-3"><span className={"text-xs font-bold " + (c.pending > 100 ? "text-red-600" : "text-gray-500")}>{c.pending}</span></td><td className="px-4 py-3"><span className="text-xs text-green-600">{c.status}</span></td></tr>))}</tbody></table></div>)}
    </div>
  );
}
