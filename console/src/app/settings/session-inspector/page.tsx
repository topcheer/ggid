  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Inspect and revoke active user sessions.</p>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") searchUser(); }} placeholder="Search user..." aria-label="Search user" className="w-full pl-8 pr-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" />
        </div>
        <button onClick={searchUser} disabled={loading || !search} aria-label="Search sessions" className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium disabled:opacity-50">{loading ? "Searching..." : "Search"}</button>
      </div>
      {loading && <div className="rounded-lg border dark:border-gray-800 p-8 text-center"><div className="inline-block w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin text-blue-600 mb-2" /><div className="text-sm text-gray-500">Searching sessions...</div></div>}
          ...
          {sessions.map((s) => (
            <div key={s.id} className={"rounded-lg border p-3 cursor-pointer " + (selected?.id === s.id ? "border-blue-500" : "dark:border-gray-800")} onClick={() => setSelected(s)}>
              ...
              <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                ...
                <span>Last: {s.last_active}</span>
              </div>
            </div>
          ))}
          {sessions.length === 0 && !loading && <p className="text-sm text-gray-500 text-center py-8">No sessions. Search a user.</p>}
        </div>
        {selected && (
          <div className="rounded-lg border dark:border-gray-800 p-4 space-y-3">
            <div className="flex items-center gap-2"><Info className="w-4 h-4 text-blue-500" /><h3 className="text-sm font-semibold">Session Details</h3></div>
            <div className="text-xs space-y-1">
              <div><span className="text-gray-500">Session ID:</span> <span className="font-mono">{selected.id}</span></div>
              <div><span className="text-gray-500">Device:</span> {selected.device}</div>
              <div><span className="text-gray-500">IP:</span> <span className="font-mono">{selected.ip_address}</span></div>
              <div><span className="text-gray-500">Location:</span> {selected.location}</div>
              <div><span className="text-gray-500">Created:</span> {selected.created_at}</div>
