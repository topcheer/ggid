...
export default function PasswordlessStatsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/auth/passwordless-stats", { headers: { "Authorization": `Bearer ${localStorage.getItem("ggid_access_token") || ""}`, "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
      ...
      {data && (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4">
              ...
              <div className="flex items-center gap-4">
                <div className="relative w-24 h-24"><svg viewBox="0 0 64 64" className="w-full h-full -rotate-90">{(() => { let offset = 0; return data.method_distribution.map((m) => { const pct = m.count / total; const dash = pct * 176; const circle = <circle key={m.method} cx={32} cy={32} r={28} fill="none" stroke={methodColors[m.method] || "#ccc"} strokeWidth={8} strokeDasharray={`${dash} 176`} strokeDashoffset={-offset * 176} />; offset += pct; return circle; }); })()}</svg><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-lg font-bold">{total}</span><span className="text-[9px] text-gray-400">total</span></div></div>
                <div className="space-y-1">{data.method_distribution.map((m) => (<div key={m.method} className="flex items-center gap-2 text-xs"><span className="w-3 h-3 rounded" style={{ background: methodColors[m.method] || "#ccc" }} /><span className="capitalize">{m.method.replace("_", " ")}</span><span className="font-bold ml-auto">{m.count}</span></div>))}</div>
              </div>
            </div>
          ...
          <div className="rounded-lg border dark:border-gray-800 p-4">
            ...
            <div className="space-y-2">{data.by_device_type.map((d) => (
