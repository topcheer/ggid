  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-4">
        ...
        <div><label className="block text-sm font-medium mb-1">{t("big1.federationPatternsConfig.discoveryMethod")}</label><select aria-label="form" value={form.discovery_method} onChange={(e) => setForm({ ...form, discovery_method: e.target.value as FederationPatternsConfig["discovery_method"] })} className="border rounded px-3 py-2"><option value="metadata_url">{t("big1.federationPatternsConfig.metadataUrl")}</option><option value="webfinger">{t("big1.federationPatternsConfig.webfinger")}</option><option value="dns">{t("big1.federationPatternsConfig.dns")}</option><option value="manual">{t("big1.federationPatternsConfig.manual")}</option></select></div>
        <div><label className="block text-sm font-medium mb-1">{t("big1.federationPatternsConfig.attributeMappingPolicy")}</label><input aria-label="form" type="text" value={form.attribute_mapping_policy} onChange={(e) => setForm({ ...form, attribute_mapping_policy: e.target.value })} className="border rounded px-3 py-2 w-full" /></div>
        <div className="flex items-center gap-3"><input aria-label="Form" type="checkbox" checked={form.slo_propagation} onChange={(e) => setForm({ ...form, slo_propagation: e.target.checked })} className="w-4 h-4" /><label>{t("big1.federationPatternsConfig.sloPropagation")}</label></div>
      </div>
      ...
      <button aria-label="action" onClick={handleSave} disabled={saving} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">{saving ? t("big1.federationPatternsConfig.saving") : t("big1.federationPatternsConfig.saveChanges")}</button>
    </div>
  );
}
