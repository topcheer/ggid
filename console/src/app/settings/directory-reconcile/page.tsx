...
export default function DirectoryReconcilePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><GitMerge className="w-6 h-6 text-indigo-500" /> Directory Reconcile</h1>
        <p className="text-sm text-gray-500 mt-1">Detect and resolve orphaned and duplicate identities across connected directories.</p>
      </div>
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4 flex items-center gap-4">
        ...
        <div className="flex items-center gap-2"><label className="text-sm font-medium">Merge Strategy:</label><select value={mergeStrategy} onChange={(e) => setMergeStrategy(e.target.value)} className="px-3 py-1.5 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm"><option value="newest">Newest Entry</option><option value="oldest">Oldest Entry</option><option value="most_complete">Most Complete</option><option value="manual">Manual Review</option></select></div>
      </div>
        ...
        <>
          {data.orphaned_ids.length > 0 && (
            <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">ID</th><th className="px-4 py-3 text-left font-medium">Type</th><th className="px-4 py-3 text-left font-medium">Source</th><th className="px-4 py-3 text-left font-medium">Last Seen</th></tr></thead>
          ...
          {data.duplicate_groups.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-sm font-semibold">Duplicate Groups</h3>
              {data.duplicate_groups.map((g) => (
                <div key={g.group_id} className="rounded-lg border dark:border-gray-800 p-4">
                  <div className="flex items-center justify-between mb-2"><span className="text-xs font-mono text-gray-400">Group: {g.group_id}</span><span className="text-xs text-green-600">Merge target: {g.suggested_merge_target}</span></div>
