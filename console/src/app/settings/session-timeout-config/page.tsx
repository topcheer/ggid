  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-6 space-y-4 max-w-lg">
        <div><label className="text-sm font-medium">{t("sessionTimeout.idleTimeout")}</label><div className="flex items-center gap-3 mt-1"><input type="range" min={5} max={120} value={config.idle_timeout_minutes} onChange={(e) => setConfig({ ...config, idle_timeout_minutes: parseInt(e.target.value) })} className="flex-1" /><span className="text-sm font-bold w-12 text-right">{config.idle_timeout_minutes}m</span></div></div>
        <div><label className="text-sm font-medium">{t("sessionTimeout.absoluteTimeout")}</label><div className="flex items-center gap-3 mt-1"><input type="range" min={1} max={24} value={config.absolute_timeout_hours} onChange={(e) => setConfig({ ...config, absolute_timeout_hours: parseInt(e.target.value) })} className="flex-1" /><span className="text-sm font-bold w-12 text-right">{config.absolute_timeout_hours}h</span></div></div>
        ...
        <div className="flex items-center gap-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={config.grace_period} onChange={(e) => setConfig({ ...config, grace_period: e.target.checked })} className="rounded" /><span className="text-sm">{t("sessionTimeout.gracePeriod")}</span></label><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={config.enforce_on_mobile} onChange={(e) => setConfig({ ...config, enforce_on_mobile: e.target.checked })} className="rounded" /><span className="text-sm">{t("sessionTimeout.enforceMobile")}</span></label></div>
      </div>
