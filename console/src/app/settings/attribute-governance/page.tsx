...
export default function AttributeGovernancePage() {
  ...
  const restricted = attrs.filter((a: any) => a.pii_classification === "restricted" || a.pii_classification === "confidential").length;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><Shield className="w-6 h-6 text-red-500" /> {t("attributeGovernance.title")}</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("backend3.attributeGovernance.totalAttributes")}</span><p className="text-xl font-bold mt-1">{attrs.length}</p></div>
        ...
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("backend3.attributeGovernance.masked")}</span><p className="text-xl font-bold text-green-600 mt-1">{attrs.filter((a: any) => a.mask_rule !== "none").length}</p></div>
      </div>

      <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
        <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">{t("backend3.attributeGovernance.attribute")}</th><th className="px-4 py-3 text-left font-medium">{t("backend3.attributeGovernance.piiClass")}</th><th className="px-4 py-3 text-left font-medium">{t("backend3.attributeGovernance.maskRule")}</th><th className="px-4 py-3 text-left font-medium">{t("backend3.attributeGovernance.accessFreq")}</th><th className="px-4 py-3 text-left font-medium">{t("backend3.attributeGovernance.lastAccessedBy")}</th><th className="px-4 py-3 text-left font-medium">{t("backend3.attributeGovernance.retention")}</th></tr></thead>
          <tbody className="divide-y dark:divide-gray-800">{attrs.map((a: any) => (
