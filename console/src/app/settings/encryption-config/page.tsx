  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <section className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div><label className="text-sm font-medium">Key Rotation Interval (days)</label><input aria-label="key Rotation" type="number" min={1} max={365} value={keyRotation} onChange={e => setKeyRotation(parseInt(e.target.value) || 90)} className="w-24 border rounded px-2 py-1 text-sm mt-1" /></div>
        <div><label className="text-sm font-medium">KMS Provider</label><select aria-label="kms Provider" value={kmsProvider} onChange={e => setKmsProvider(e.target.value)} className="w-full border rounded px-3 py-2 text-sm mt-1"><option value="internal">Internal (built-in)</option><option value="aws-kms">AWS KMS</option><option value="gcp-kms">Google Cloud KMS</option><option value="azure-kv">Azure Key Vault</option><option value="hashicorp-vault">HashiCorp Vault</option></select></div>
        <label className="flex items-center justify-between"><span className="text-sm">Envelope Encryption</span><input aria-label="Envelope encryption" type="checkbox" checked={envelopeEncryption} onChange={e => setEnvelopeEncryption(e.target.checked)} className="rounded" /></label>
        <label className="flex items-center justify-between"><span className="text-sm">HSM Integration</span><input aria-label="Hsm enabled" type="checkbox" checked={hsmEnabled} onChange={e => setHsmEnabled(e.target.checked)} className="rounded" /></label>
        <label className="flex items-center justify-between"><span className="text-sm">Key Backup</span><input aria-label="Key backup" type="checkbox" checked={keyBackup} onChange={e => setKeyBackup(e.target.checked)} className="rounded" /></label>
      </section>
      ...
      <section className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div><label className="text-sm font-medium">Cipher Suites</label><div className="space-y-2 mt-2">{allCiphers.map(c => <label key={c} className="flex items-center gap-2 text-sm"><input aria-label="Cipher suites" type="checkbox" checked={cipherSuites.includes(c)} onChange={() => toggleCipher(c)} className="rounded" /><span className="font-mono text-xs">{c}</span></label>)}</div></div>
      </section>
    </div>
  );
}
