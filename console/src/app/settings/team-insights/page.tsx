...
export default function TeamInsightsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/identity/team-insights", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  const teams = data ? [...new Set(data.expertise_distribution.map((e: any) => e.team))] : [];
  const skills = data ? [...new Set(data.expertise_distribution.map((e: any) => e.skill))] : [];
      ...
      {data && (
        <>
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1 space-y-1">{data.silo_detection.filter((s: any) => s.isolation_pct > 50).map((s: any) => (<div key={s.team} className="flex items-center gap-2 text-sm"><AlertTriangle className="w-4 h-4 text-red-500" /><span className="font-medium">{s.team}</span><span className="text-red-600">{s.isolation_pct}% isolated</span></div>))}</div>
          </div>

          <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Collaboration Heatmap</h3><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">{data.collaboration_patterns.map((c: any, i: number) => { const intensity = Math.min(c.frequency / 100, 1); return (<div key={i} className="rounded p-2 text-xs" style={{ background: `rgba(16, 185, 129, ${intensity * 0.8 + 0.1})` }}><span className="font-medium block">{c.team_a} / {c.team_b}</span><span className="text-gray-600 dark:text-gray-300">{c.frequency} interactions</span></div>); })}</div></div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Cross-Team Dependencies</h3><div className="space-y-1">{data.cross_team_deps.map((d: any, i: number) => (<div key={i} className="flex items-center gap-2 text-sm"><span className="font-mono text-xs">{d.from}</span><span className="text-gray-400">{"->"}</span><span className="font-mono text-xs">{d.to}</span><span className="px-1.5 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-800">{d.type}</span></div>))}{data.cross_team_deps.length === 0 && <p className="text-xs text-gray-400">None detected.</p>}</div></div>

            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold flex items-center gap-2 mb-3"><TrendingDown className="w-4 h-4 text-red-500" /> Attrition Risk</h3><div className="space-y-1">{data.attrition_risk.map((a: any) => (<div key={a.team} className="flex items-center gap-2 text-sm"><span className="flex-1">{a.team}</span><span className={`px-2 py-0.5 rounded text-xs ${riskColors[a.risk_level]}`}>{a.risk_level}</span></div>))}</div></div>
          </div>

          {skills.length > 0 && teams.length > 0 && (
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Expertise Distribution</h3><div className="space-y-2">{skills.map((skill: any) => (<div key={skill} className="flex items-center gap-2"><span className="text-xs text-gray-500 w-24">{skill}</span><div className="flex-1 flex items-center gap-1">{teams.map((team: any) => { const exp = data.expertise_distribution.find((e: any) => e.team === team && e.skill === skill); return <div key={team} className="flex-1"><div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-4 overflow-hidden"><div className="h-full bg-teal-500 rounded-full" style={{ width: `${exp?.level ?? 0}%` }} /></div><span className="text-[9px] text-gray-400 block text-center mt-0.5 truncate">{team}</span></div>; })}</div></div>))}</div></div>
          )}
        </>
      )}
