...
export default function OidcBackchannelLogoutConfigPage() {
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      <h1 className="text-2xl font-bold">OIDC Backchannel Logout Configuration</h1>
      <p className="text-gray-600">Configure backchannel logout (RFC draft) per client.</p>

      <div className="bg-white rounded-lg p-6 shadow space-y-4">
        <h2 className="text-lg font-semibold">Global Settings</h2>
        <div>
          <label className="block text-sm font-medium mb-1">Session Lifetime After Logout (s)</label>
        ...
        <div className="flex items-center gap-3">
          <input type="checkbox" checked={form.token_revocation_on_logout} onChange={(e) => setForm({ ...form, token_revocation_on_logout: e.target.checked })} className="w-4 h-4" />
          <label>Token Revocation on Logout</label>
        </div>
      </div>

      <div className="bg-white rounded-lg p-6 shadow">
        <h2 className="text-lg font-semibold mb-4">Per-Client Logout Endpoints</h2>
        <table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="py-2">Client</th><th>Logout Endpoint URL</th><th>Test</th></tr></thead><tbody>
      ...
      <div className="bg-white rounded-lg p-6 shadow">
        <h2 className="text-lg font-semibold mb-4">Logout Token Preview</h2>
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-3">
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div><span className="text-gray-500">Retry Attempts:</span> <span className="font-medium">{form.error_handling.retry_attempts}</span></div>
          <div><span className="text-gray-500">Timeout (s):</span> <span className="font-medium">{form.error_handling.timeout_seconds}</span></div>
          <div><span className="text-gray-500">Failed (24h):</span> <span className="font-medium text-red-600">{form.error_handling.failed_24h}</span></div>
        </div>
      </div>
