...
export default function ConditionBuilderPage() {
  ...
  const addCondition = (gid: string) => { setGroups(groups.map((g: any) => g.id === gid ? { ...g, conditions: [...g.conditions, { id: "c" + Date.now(), attribute: "user.role", operator: "eq", value: "" }] } : g)); };
  const removeCondition = (gid: string, cid: string) => { setGroups(groups.map((g: any) => g.id === gid ? { ...g, conditions: g.conditions.filter((c: any) => c.id !== cid) } : g)); };
  const updateCondition = (gid: string, cid: string, field: string, value: string) => { setGroups(groups.map((g: any) => g.id === gid ? { ...g, conditions: g.conditions.map((c: any) => c.id === cid ? { ...c, [field]: value } : c) } : g)); };
  const addGroup = () => { setGroups([...groups, { id: "g" + Date.now(), logic: "AND", conditions: [] }]); };
  const toggleLogic = (gid: string) => { setGroups(groups.map((g: any) => g.id === gid ? { ...g, logic: g.logic === "AND" ? "OR" : "AND" } : g)); };
  const jsonPreview = JSON.stringify(groups.map((g: any) => ({ logic: g.logic, conditions: g.conditions.map((c: any) => ({ attribute: c.attribute, operator: c.operator, value: c.value })) })), null, 2);
