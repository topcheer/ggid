      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setEditing(null)}>
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-bold text-gray-900 dark:text-white">{editing.id ? "Edit Rule" : "New ACL Rule"}</h3><button onClick={() => setEditing(null)}><X className="h-5 w-5 text-gray-400" /></button></div>
            <div className="space-y-4">
              <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-400">Resource Path</label><input value={editing.resource_path} onChange={(e) => setEditing({ ...editing, resource_path: e.target.value })} placeholder="/api/v1/users/*" className="w-full rounded-lg border border-gray-300 px-3 py-2 font-mono text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /></div>
              <div className="flex gap-4">
                <div className="flex-1"><label className="mb-1 block text-xs font-semibold uppercase text-gray-400">Principal</label><input value={editing.principal} onChange={(e) => setEditing({ ...editing, principal: e.target.value })} placeholder="role:admin" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /></div>
                <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-400">Type</label><select value={editing.principal_type} onChange={(e) => setEditing({ ...editing, principal_type: e.target.value as ACLRule["principal_type"] })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"><option value="user">User</option><option value="role">Role</option><option value="group">Group</option><option value="service">Service</option></select></div>
              ...
              <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-400">Permissions (comma-separated)</label><input value={editing.permissions.join(", ")} onChange={(e) => setEditing({ ...editing, permissions: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} placeholder="read, write" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /></div>
              <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-400">Conditions</label><input value={editing.conditions} onChange={(e) => setEditing({ ...editing, conditions: e.target.value })} placeholder="time >= 9am AND time <= 17pm" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /></div>
