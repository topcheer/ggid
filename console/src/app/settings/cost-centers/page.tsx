...
export default function CostCentersPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/identity/cost-centers", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div className="rounded-lg border dark:border-gray-800 p-4">
              ...
              <div className="flex items-center gap-2 mt-2"><svg viewBox="0 0 64 64" className="w-12 h-12 -rotate-90">{(() => { let off = 0; return data.allocation.map((a: any, i: number) => { const pct = a.amount / totalAlloc; const dash = pct * 176; const c = <circle key={i} cx={32} cy={32} r={28} fill="none" stroke={allocColors[i % allocColors.length]} strokeWidth={8} strokeDasharray={`${dash} 176`} strokeDashoffset={-off * 176} />; off += pct; return c; }); })()}</svg></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.departments.map((d: any) => (
              <div key={d.id} className="rounded-lg border dark:border-gray-800 p-4 space-y-3">
                ...
                {d.alerts.length > 0 && <div className="space-y-1">{d.alerts.map((a: any, i: number) => <div key={i} className="text-xs text-red-500">- {a}</div>)}</div>}
              </div>
            ))}
          </div>
        </>
