...
export default function CostCentersPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/identity/cost-centers", { headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><DollarSign className="w-6 h-6 text-green-500" /> Cost Centers</h1>
      ...
      {data && (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            <div className="rounded-lg border dark:border-gray-800 p-4 flex items-center gap-3"><Building2 className="w-8 h-8 text-blue-500" /><div><span className="text-sm text-gray-500">Departments</span><p className="text-xl font-bold mt-1">{data.departments.length}</p></div></div>
            <div className="rounded-lg border dark:border-gray-800 p-4 flex items-center gap-3"><DollarSign className="w-8 h-8 text-green-500" /><div><span className="text-sm text-gray-500">Total Budget</span><p className="text-xl font-bold mt-1">${totalBudget.toLocaleString()}</p></div></div>
            <div className="rounded-lg border dark:border-gray-800 p-4 flex items-center gap-3"><DollarSign className="w-8 h-8 text-orange-500" /><div><span className="text-sm text-gray-500">Used</span><p className="text-xl font-bold text-orange-600 mt-1">${totalUsed.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p></div></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.departments.map((d) => (
              <div key={d.id} className="rounded-lg border dark:border-gray-800 p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm"><div className="flex items-center gap-1"><Users className="w-3.5 h-3.5 text-gray-400" /><span className="text-gray-500">Members</span><span className="font-bold ml-auto">{d.member_count}</span></div><div className="flex items-center gap-1"><DollarSign className="w-3.5 h-3.5 text-gray-400" /><span className="text-gray-500">Budget</span><span className="font-bold ml-auto">${d.budget.toLocaleString()}</span></div></div>
                <div><div className="flex items-center justify-between text-xs mb-1"><span className="text-gray-500">Resource Usage</span><span className={`font-bold ${d.budget_used_pct >= 90 ? "text-red-600" : d.budget_used_pct >= 70 ? "text-orange-600" : "text-green-600"}`}>{d.budget_used_pct.toFixed(0)}%</span></div><div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-2 overflow-hidden"><div className="h-full rounded-full" style={{ width: `${d.budget_used_pct}%`, background: d.budget_used_pct >= 90 ? "#ef4444" : d.budget_used_pct >= 70 ? "#f59e0b" : "#10b981" }} /></div></div>
