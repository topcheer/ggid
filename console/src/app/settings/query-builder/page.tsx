...
export default function QueryBuilderPage() {
  ...
  const removeWhere = (id: string) => { setWhereClauses(whereClauses.filter((w: any) => w.id !== id)); };
  const updateWhere = (id: string, field: string, value: string) => { setWhereClauses(whereClauses.map((w: any) => w.id === id ? { ...w, [field]: value } : w)); };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4 space-y-4">
        <div><label className="text-sm font-medium">SELECT</label><div className="flex flex-wrap gap-1 mt-1">{fields.map((f: any) => <label key={f} className="flex items-center gap-1 text-xs"><input aria-label="Select fields" type="checkbox" checked={selectFields.includes(f)} onChange={(e) => { if (e.target.checked) setSelectFields([...selectFields, f]); else setSelectFields(selectFields.filter((s: any) => s !== f)); }} /> {f}</label>)}</div></div>
        <div><label className="text-sm font-medium">WHERE</label><div className="space-y-2 mt-1">{whereClauses.map((w: any) => (<div key={w.id} className="flex items-center gap-2"><select value={w.field} onChange={(e) => updateWhere(w.id, "field", e.target.value)} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs font-mono">{fields.map((f: any) => <option key={f} value={f}>{f}</option>)}</select><select value={w.operator} onChange={(e) => updateWhere(w.id, "operator", e.target.value)} className="px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs">{operators.map((o: any) => <option key={o} value={o}>{o}</option>)}</select><input aria-label="value" type="text" value={w.value} onChange={(e) => updateWhere(w.id, "value", e.target.value)} placeholder="value" className="flex-1 px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs font-mono" /><button onClick={() => removeWhere(w.id)} className="text-red-500"><X className="w-4 h-4" /></button></div>))}<button onClick={addWhere} className="text-xs text-blue-600 flex items-center gap-1"><Plus className="w-3 h-3" /> Add</button></div></div>
        <div className="grid grid-cols-3 gap-3"><div><label className="text-sm font-medium">GROUP BY</label><select value={groupBy} onChange={(e) => setGroupBy(e.target.value)} className="w-full mt-1 px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs"><option value="">None</option>{fields.map((f: any) => <option key={f} value={f}>{f}</option>)}</select></div><div><label className="text-sm font-medium">ORDER BY</label><select value={orderBy} onChange={(e) => setOrderBy(e.target.value)} className="w-full mt-1 px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs">{fields.map((f: any) => <option key={f} value={f}>{f}</option>)}</select></div><div><label className="text-sm font-medium">LIMIT</label><input aria-label="group By" type="number" value={limit} onChange={(e) => setLimit(parseInt(e.target.value) || 50)} className="w-full mt-1 px-2 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs" /></div></div>
      ...
      {history.length > 0 && <div className="rounded-lg border dark:border-gray-800 p-3"><h4 className="text-xs font-semibold text-gray-500 mb-1">Recent Queries</h4><div className="space-y-1">{history.map((h: any, i: number) => <div key={i} className="text-xs font-mono text-gray-400 truncate">{h}</div>)}</div></div>}
      {results.length > 0 && <div className="overflow-x-auto rounded-lg border dark:border-gray-800"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr>{selectFields.map((f: any) => <th key={f} className="px-4 py-3 text-left font-medium">{f}</th>)}</tr></thead><tbody className="divide-y dark:divide-gray-800">{results.map((r: any, i: number) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3 text-xs">{r.timestamp}</td><td className="px-4 py-3 text-xs font-mono">{r.user}</td><td className="px-4 py-3 text-xs">{r.action}</td><td className="px-4 py-3 text-xs">{r.resource}</td></tr>))}</tbody></table></div>}
    </div>
  );
}
