...
export default function AudienceMismatchesPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/oauth/audience-mismatches", { headers: { "Authorization": `Bearer ${localStorage.getItem("ggid_access_token") || ""}`, "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2"><ShieldAlert className="w-6 h-6 text-orange-500" /> {t("audienceMismatches.title")}</h1>
        <p className="text-sm text-gray-500 mt-1">Track tokens presented with incorrect audience claims.</p>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("backend3.audienceMismatches.totalMismatches")}</span><p className="text-xl font-bold mt-1">{mismatches.length}</p></div>
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("backend3.audienceMismatches.blocked")}</span><p className="text-xl font-bold text-red-600 mt-1">{blockedCount}</p></div>
