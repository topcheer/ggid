...
export default function OAuthParUsagePage() {
  ...
  if (loading) return <div className="p-8 text-gray-400">{t("oauthParUsage.loading")}</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gray-900 rounded-xl p-4"><Database className="w-5 h-5 text-blue-400 mb-1" /><p className="text-xs text-gray-400">{t("oauthParUsage.totalPushed")}</p><p className="text-xl font-bold">{(data?.total_pushed ?? 0).toLocaleString()}</p></div>
        <div className="bg-gray-900 rounded-xl p-4"><Activity className="w-5 h-5 text-green-400 mb-1" /><p className="text-xs text-gray-400">{t("oauthParUsage.activeRequests")}</p><p className="text-xl font-bold">{data?.active_requests?.length ?? 0}</p></div>
        <div className="bg-gray-900 rounded-xl p-4 text-center"><Zap className="w-5 h-5 text-purple-400 mx-auto mb-1" /><p className="text-xs text-gray-400">{t("oauthParUsage.hitRate")}</p><p className="text-xl font-bold text-green-400">{data?.hit_rate ?? 0}%</p></div>
        <div className="bg-gray-900 rounded-xl p-4"><p className="text-xs text-gray-400">{t("oauthParUsage.cacheSize")}</p><p className="text-xl font-bold">{(data?.cache_size ?? 0).toLocaleString()}</p></div>
      </div>
