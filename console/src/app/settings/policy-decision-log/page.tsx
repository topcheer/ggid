...
export default function PolicyDecisionLogPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await fetch("/api/v1/policy/decision-log", { headers: { "Authorization": `Bearer ${localStorage.getItem("ggid_access_token") || ""}`, "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); if (res.ok) { const d = await res.json(); setDecisions(d.decisions || d || []); } }
    catch { /* noop */ }
  ...
  const filtered = decisions.filter((d) => { if (filterDecision && d.decision !== filterDecision) return false; if (filterPolicy && !d.policy_id.includes(filterPolicy)) return false; return true; });
  const policies = [...new Set(decisions.map((d) => d.policy_id))];

  const exportCSV = () => { const csv = ["timestamp,policy_id,subject,resource,action,decision,matched_rules,eval_ms", ...filtered.map((d) => [d.timestamp, d.policy_id, d.subject, d.resource, d.action, d.decision, d.matched_rules.join(";"), d.evaluation_time_ms].join(","))].join("\n"); const blob = new Blob([csv], { type: "text/csv" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "policy-decisions.csv"; a.click(); };
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
        <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Timestamp</th><th className="px-4 py-3 text-left font-medium">Policy</th><th className="px-4 py-3 text-left font-medium">Subject</th><th className="px-4 py-3 text-left font-medium">Resource</th><th className="px-4 py-3 text-left font-medium">Action</th><th className="px-4 py-3 text-left font-medium">Decision</th><th className="px-4 py-3 text-left font-medium">Rules</th><th className="px-4 py-3 text-left font-medium">Time</th></tr></thead>
          <tbody className="divide-y dark:divide-gray-800">{filtered.map((d) => (<tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3 text-xs text-gray-400">{d.timestamp}</td><td className="px-4 py-3 font-mono text-xs">{d.policy_id}</td><td className="px-4 py-3 font-mono text-xs">{d.subject}</td><td className="px-4 py-3 font-mono text-xs">{d.resource}</td><td className="px-4 py-3 text-xs">{d.action}</td><td className="px-4 py-3">{d.decision === "allow" ? <span className="flex items-center gap-1 text-xs text-green-600"><ShieldCheck className="w-3.5 h-3.5" /> allow</span> : <span className="flex items-center gap-1 text-xs text-red-600"><ShieldX className="w-3.5 h-3.5" /> deny</span>}</td><td className="px-4 py-3"><div className="flex flex-wrap gap-1">{d.matched_rules.slice(0, 2).map((r, i) => <span key={i} className="px-1.5 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-800 font-mono">{r}</span>)}{d.matched_rules.length > 2 && <span className="text-xs text-gray-400">+{d.matched_rules.length - 2}</span>}</div></td><td className="px-4 py-3 text-xs text-gray-500">{d.evaluation_time_ms}ms</td></tr>))}{filtered.length === 0 && !loading && <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500">No decisions logged.</td></tr>}</tbody>
        </table>
      </div>
    </div>
  );
