...
export default function DatabaseHealthPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold flex items-center gap-2"><Database className="w-6 h-6 text-blue-500" /> Database Health</h1><p className="text-sm text-gray-500 mt-1">{t("databaseHealth.subtitle")}</p></div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("databaseHealth.queryRate")}</span><p className="text-xl font-bold mt-1">{data.query_rate_per_sec}/s</p></div>
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("databaseHealth.avgLatency")}</span><p className={"text-xl font-bold mt-1 " + (data.avg_latency_ms > 100 ? "text-red-600" : "text-green-600")}>{data.avg_latency_ms}ms</p></div>
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("databaseHealth.indexEfficiency")}</span><p className="text-xl font-bold mt-1">{data.index_efficiency_pct}%</p></div>
        <div className="rounded-lg border p-4 dark:border-gray-800"><span className="text-sm text-gray-500">{t("databaseHealth.replLag")}</span><p className={"text-xl font-bold mt-1 " + (data.replication_lag_seconds > 5 ? "text-red-600" : "text-green-600")}>{data.replication_lag_seconds}s</p></div>
      </div>

      <div className="rounded-lg border dark:border-gray-800 p-4"><div className="flex items-center justify-between mb-3"><h3 className="text-sm font-semibold">{t("databaseHealth.connectionPool")}</h3><span className="text-sm text-gray-500">{data.pool_active}/{data.pool_max} active</span></div><div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-6 overflow-hidden"><div className={"h-full rounded-full " + (poolPct > 80 ? "bg-red-500" : poolPct > 60 ? "bg-yellow-500" : "bg-green-500")} style={{ width: poolPct + "%" }} /></div><div className="flex justify-between mt-2 text-xs text-gray-500"><span>{t("databaseHealth.active")} {data.pool_active}</span><span>{t("databaseHealth.idle")} {data.pool_idle}</span><span>{t("databaseHealth.max")} {data.pool_max}</span></div>{poolPct > 80 && <div className="mt-2 flex items-center gap-2 text-xs text-red-600"><AlertTriangle className="w-3.5 h-3.5" /> Pool utilization above 80%</div>}</div>
