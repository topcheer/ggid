...
export default function LoginAnalyticsPage() {
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/v1/auth/login-analytics?start=${startDate}&end=${endDate}`, { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <button aria-label="action" onClick={fetchAnalytics} disabled={loading} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50">{loading ? "Loading..." : "Refresh"}</button>
      </div>
