        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className={card}>
            ...
            <div className="space-y-4">
              <div><label className="text-sm font-medium">{t("branding.primaryColor")}</label><div className="mt-1 flex items-center gap-3"><input type="color" aria-label={t("branding.primaryColor")} value={primaryColor} onChange={e => setPrimaryColor(e.target.value)} className="h-10 w-16 rounded cursor-pointer" /><code className="text-xs font-mono">{primaryColor}</code></div></div>
              <div><label className="text-sm font-medium">{t("branding.accentColor")}</label><div className="mt-1 flex items-center gap-3"><input type="color" aria-label={t("branding.accentColor")} value={accentColor} onChange={e => setAccentColor(e.target.value)} className="h-10 w-16 rounded cursor-pointer" /><code className="text-xs font-mono">{accentColor}</code></div></div>
              <div><label className="text-sm font-medium flex items-center gap-2"><Type className="h-4 w-4" /> {t("branding.fontFamily")}</label><select aria-label={t("branding.fontFamily")} value={fontFamily} onChange={e => setFontFamily(e.target.value)} className="mt-1 block w-full rounded-lg border dark:border-gray-700 dark:bg-gray-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{fonts.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
              <div><label className="text-sm font-medium flex items-center gap-2"><Square className="h-4 w-4" /> {t("branding.borderRadius")}</label><div className="mt-1 flex items-center gap-3"><input type="range" aria-label={t("branding.borderRadius")} min={0} max={20} value={borderRadius} onChange={e => setBorderRadius(parseInt(e.target.value))} className="flex-1 accent-pink-500" /><span className="text-sm font-mono w-8">{borderRadius}px</span></div></div>
            ...
            <button onClick={save} disabled={saving} className="mt-4 flex items-center gap-2 rounded-lg bg-pink-600 px-4 py-2 text-sm font-medium text-white hover:bg-pink-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} {t("branding.saveTheme")}</button>
          </div>
          {/* Live preview */}
          <div className={card}>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className={card}>
              ...
              <div className="flex items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-700 dark:bg-gray-700 text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"><Image className="h-8 w-8" /></div><button className="rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm dark:border-gray-700 flex items-center gap-2"><Upload className="h-3.5 w-3.5" /> {t("branding.upload")}</button></div>
          ...
          <div className={card}>
            ...
            <textarea aria-label="Custom CSS" rows={8} placeholder={"/* Custom CSS injected into console */\n:root {\n  --brand-gradient: linear-gradient(...);\n}"} className="w-full rounded-lg border dark:border-gray-700 dark:bg-gray-900 px-3 py-2 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-blue-500" />
