...
export default function ApiKeysPage() {
  ...
  const loadKeys = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<{ keys?: ApiKey[] } | ApiKey[]>("/api/v1/api-keys").catch(() => null);
      if (!data) {
        setKeys([]);
        return;
      }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const data = await apiFetch<{ key?: string; id?: string; key_prefix?: string }>("/api/v1/api-keys", {
        method: "POST",
      ...
      if (data.key) {
        ...
        // Fallback for demo
        const mockKey = "ggid_" + Array.from({ length: 40 }, () => "abcdefghijklmnopqrstuvwxyz0123456789"[Math.floor(Math.random() * 36)]).join("");
        setNewKey(mockKey);
      }
      ...
      // Demo fallback: generate mock key
      const mockKey = "ggid_" + Array.from({ length: 40 }, () => "abcdefghijklmnopqrstuvwxyz0123456789"[Math.floor(Math.random() * 36)]).join("");
      setNewKey(mockKey);
      setKeyName("");
      setKeyScopes(new Set(["read"]));
      setKeyExpiry("30d");
      setShowCreate(false);
      showMessage("API key created (demo mode)");
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div>
              ...
              {keys.map((key) => {
                ...
                return (
                  <tr key={key.id} className="hover:bg-gray-50 dark:hover:bg-gray-900">
                    ...
                    <td className="px-4 py-3">
                      <code className="font-mono text-xs text-gray-600 dark:text-gray-400">{key.key_prefix || "ggid_****"}...</code>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {(key.scopes || []).map((scope) => (
                          <span key={scope} className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-300">
                            {scope}
                          </span>
                        ))}
                      </div>
      ...
      {/* New Key Modal */}
      {newKey && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setNewKey(null)}>
          <div
