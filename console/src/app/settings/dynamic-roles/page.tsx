        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setEditing(null)}>
          <div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-400">{t("big1.dynamicRoles.conditions")}</label><div className="space-y-2">{editing.conditions.map((c, idx) => (<div key={idx} className="flex items-center gap-2"><input aria-label="attribute" value={c.attribute} onChange={(e) => updateCondition(idx, "attribute", e.target.value)} placeholder="attribute" className="flex-1 rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /><select value={c.operator} onChange={(e) => updateCondition(idx, "operator", e.target.value)} className="rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"><option value="eq">==</option><option value="ne">!=</option><option value="in">{t("big1.dynamicRoles.in")}</option><option value="contains">{t("big1.dynamicRoles.contains")}</option></select><input value={c.value} onChange={(e) => updateCondition(idx, "value", e.target.value)} placeholder="value" className="flex-1 rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /><button onClick={() => removeCondition(idx)} className="text-red-400"><Trash2 className="h-3 w-3" /></button></div>))}</div><button onClick={addCondition} className="mt-2 flex items-center gap-1 text-xs text-purple-600 hover:underline"><Plus className="h-3 w-3" />{t("big1.dynamicRoles.addCondition")}</button></div>
