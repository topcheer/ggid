            ...
            <div key={r.id} className={cardCls}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">{r.conditions.map((c, i) => <span key={i} className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-xs text-gray-600 dark:bg-gray-700 dark:text-gray-300">{c.attribute} {c.operator} {c.value}</span>)}{r.conditions.length === 0 && <span className="text-xs text-gray-400">{t("big1.dynamicRoles.noConditions")}</span>}</div>
                </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setEditing(null)}>
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-bold text-gray-900 dark:text-white">{editing.id ? t("big1.dynamicRoles.editDynamicRole") : t("big1.dynamicRoles.newDynamicRole")}</h3><button onClick={() => setEditing(null)}><X className="h-5 w-5 text-gray-400" /></button></div>
            <div className="space-y-4">
              ...
              <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-400">{t("big1.dynamicRoles.conditions")}</label><div className="space-y-2">{editing.conditions.map((c, idx) => (<div key={idx} className="flex items-center gap-2"><input value={c.attribute} onChange={(e) => updateCondition(idx, "attribute", e.target.value)} placeholder="attribute" className="flex-1 rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /><select value={c.operator} onChange={(e) => updateCondition(idx, "operator", e.target.value)} className="rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"><option value="eq">==</option><option value="ne">!=</option><option value="in">{t("big1.dynamicRoles.in")}</option><option value="contains">{t("big1.dynamicRoles.contains")}</option></select><input value={c.value} onChange={(e) => updateCondition(idx, "value", e.target.value)} placeholder="value" className="flex-1 rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200" /><button onClick={() => removeCondition(idx)} className="text-red-400"><Trash2 className="h-3 w-3" /></button></div>))}</div><button onClick={addCondition} className="mt-2 flex items-center gap-1 text-xs text-purple-600 hover:underline"><Plus className="h-3 w-3" />{t("big1.dynamicRoles.addCondition")}</button></div>
              ...
              <button onClick={handleSave} className="flex w-full items-center justify-center gap-2 rounded-lg bg-purple-600 py-2 text-sm font-medium text-white hover:bg-purple-700">{editing.id ? t("big1.dynamicRoles.update") : t("big1.dynamicRoles.create")}{t("big1.dynamicRoles.dynamicRole")}</button>
            </div>
          </div>
        </div>
      )}
