...
export default function PlatformAccessPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      <h1 className="mb-1 text-2xl font-bold text-gray-900 dark:text-white">Platform Access</h1>
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={copyRequestLink} className="flex items-center gap-1.5 rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700">
      ...
      {showGrant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl bg-white p-6 dark:bg-gray-900 mx-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={scope} onChange={e => setScope(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800">
              ...
              <div>
                ...
                <select value={expiresIn} onChange={e => setExpiresIn(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800">
              ...
              <div>
                ...
                <textarea value={reason} onChange={e => setReason(e.target.value)} required rows={3} placeholder="e.g., Investigating user login issues reported on 2024-01-15" className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" />
              </div>
            </div>
            <div className="mt-6 flex justify-end gap-2">
              <button onClick={() => setShowGrant(false)} className="rounded-lg border border-gray-300 px-4 py-2 text-sm dark:border-gray-700">Cancel</button>
            ...
            consents.filter(c => !c.revoked_at).map(c => (
              <div key={c.id} className="flex items-center justify-between px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{c.granted_to}</p>
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {consents.filter(c => c.revoked_at).map(c => (
              <div key={c.id} className="flex items-center gap-3 px-6 py-3 opacity-60">
                ...
                <span className="text-sm text-gray-600 dark:text-gray-400">{c.granted_to} — revoked {c.revoked_at ? new Date(c.revoked_at).toLocaleDateString() : ""}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            logs.map(l => (
              <div key={l.id} className="flex items-center justify-between px-6 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{l.impersonator_id.substring(0, 8)}</p>
