...
export default function PolicyChangeHistoryPage() {
  ...
  const toggleCompare = (version: string) => { setSelectedVersions(selectedVersions.includes(version) ? selectedVersions.filter((v: any) => v !== version) : selectedVersions.length < 2 ? [...selectedVersions, version] : [selectedVersions[1], version]); };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <select aria-label="Policy id" value={policyId} onChange={(e) => setPolicyId(e.target.value)} className="px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm"><option value="">Select Policy</option>{policies.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
