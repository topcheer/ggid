  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4 space-y-3">
        ...
        <div><label className="text-sm font-medium">Reason</label><input type="text" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Suspected misconfiguration" className="w-full mt-1 px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" /></div>
        <div><label className="text-sm font-medium">Duration: {duration} hours</label><input type="range" min={1} max={168} value={duration} onChange={(e) => setDuration(parseInt(e.target.value))} className="w-full mt-2 accent-red-500" /><div className="flex justify-between text-xs text-gray-400 mt-1"><span>1h</span><span>24h</span><span>168h (7d)</span></div></div>
        <button onClick={quarantine} disabled={loading || !policyId} className="px-4 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 disabled:opacity-50 flex items-center gap-2"><ShieldOff className="w-4 h-4" /> {loading ? "Processing..." : "Quarantine Policy"}</button>
      </div>
