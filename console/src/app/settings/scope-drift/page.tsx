  ...
  return (
    <div className="space-y-6">
      ...
      {data && (<>
        ...
        <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-2">Cleanup Recommendations</h3><div className="space-y-1">{data.recommendations.map((r, i) => (<div key={i} className="text-sm text-gray-500 flex items-start gap-2"><Trash2 className="w-3.5 h-3.5 text-gray-400 mt-0.5 flex-shrink-0" /> {r}</div>))}</div></div>
      </>)}
