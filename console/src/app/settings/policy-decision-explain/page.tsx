  ...
  return (
    <div className="space-y-6">
      ...
      {result && (<>
        ...
        <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Matched Rules</h3><div className="space-y-2">{result.matched_rules.map((r: any, i: number) => (<div key={i} className="flex items-center gap-3 text-sm"><span className={"px-2 py-0.5 rounded text-xs font-medium " + (r.effect === "allow" ? "bg-green-100 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 dark:bg-red-900/30 dark:text-red-400")}>{r.effect}</span><span className="font-mono text-xs flex-1">{r.rule}</span><span className="text-xs text-gray-400">P{r.priority}</span></div>))}</div></div>

        <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-2">Contributing Factors</h3><div className="flex flex-wrap gap-2">{result.contributing_factors.map((f: any, i: number) => (<span key={i} className="px-2 py-1 rounded text-xs bg-gray-100 dark:bg-gray-800">{f}</span>))}</div></div>

        {result.alternatives.length > 0 && (<div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-2">Alternative Policies</h3><div className="space-y-1">{result.alternatives.map((a: any, i: number) => (<div key={i} className="flex items-center gap-2 text-sm"><span className="font-mono text-xs">{a.policy}</span><span className={"text-xs " + (a.decision === "allow" ? "text-green-600" : "text-red-600")}>{a.decision}</span></div>))}</div></div>)}

        <div className="rounded-lg border dark:border-gray-800 p-4"><button onClick={() => setExpanded(!expanded)} className="flex items-center gap-2 text-sm font-semibold"><ChevronRight className={"w-4 h-4 transition-transform " + (expanded ? "rotate-90" : "")} /> Evaluation Path</button>{expanded && (<div className="mt-2 space-y-1">{result.eval_path.map((p: any, i: number) => (<div key={i} className="text-xs font-mono text-gray-500 pl-6">{i + 1}. {p}</div>))}</div>)}</div>
      </>)}
    </div>
  );
}
