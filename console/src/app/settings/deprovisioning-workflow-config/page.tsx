    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
          ...
          {steps.map((s, idx) => (
            <div key={s.id} className="flex items-center gap-3 border rounded p-3">
              ...
              <label className="flex items-center gap-1 text-sm">
                <input aria-label="S" type="checkbox" checked={s.enabled} onChange={() => toggleStep(s.id)} className="rounded" />
      ...
      <div className="grid grid-cols-2 gap-4">
        <label className="flex items-center justify-between bg-white rounded-lg shadow p-4">
          ...
          <input aria-label="Rollback" type="checkbox" checked={rollback} onChange={e => setRollback(e.target.checked)} className="rounded" />
        </label>
        <label className="flex items-center justify-between bg-white rounded-lg shadow p-4">
          <span className="text-sm font-medium">Dry-Run Mode</span>
          <input aria-label="Dry run" type="checkbox" checked={dryRun} onChange={e => setDryRun(e.target.checked)} className="rounded" />
        </label>
      </div>
      ...
      <section className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div className="space-y-2">
          <label className="flex items-center justify-between">
            ...
            <input aria-label="Notify user" type="checkbox" checked={notifyUser} onChange={e => setNotifyUser(e.target.checked)} className="rounded" />
          </label>
          <label className="flex items-center justify-between">
            <span className="text-sm">Notify Manager</span>
            <input aria-label="Notify manager" type="checkbox" checked={notifyManager} onChange={e => setNotifyManager(e.target.checked)} className="rounded" />
          </label>
          <label className="flex items-center justify-between">
            <span className="text-sm">Notify Admin</span>
            <input aria-label="Notify admin" type="checkbox" checked={notifyAdmin} onChange={e => setNotifyAdmin(e.target.checked)} className="rounded" />
          </label>
        </div>
      </section>
      ...
      <section className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <input aria-label="cleanup Days" type="number" min={30} max={365} value={cleanupDays} onChange={e => setCleanupDays(parseInt(e.target.value) || 90)} className="w-24 border rounded px-2 py-1 text-sm" />
