        ...
        <>
          ...
          <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
            <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Client</th><th className="px-4 py-3 text-left font-medium">PKCE Required</th><th className="px-4 py-3 text-left font-medium">Challenge Method</th></tr></thead>
              <tbody className="divide-y dark:divide-gray-800">{config.per_client.map((c: any, i: number) => (<tr key={c.client_id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3"><span className="font-medium">{c.client_name}</span><p className="text-xs text-gray-400 font-mono">{c.client_id}</p></td><td className="px-4 py-3"><label className="flex items-center gap-2"><input type="checkbox" checked={c.required} onChange={(e) => { const o = [...config.per_client]; o[i] = { ...c, required: e.target.checked }; setConfig({ ...config, per_client: o }); }} className="rounded" /><span className="text-xs">{c.required ? "Yes" : "No"}</span></label></td><td className="px-4 py-3"><select aria-label="Select option" value={c.challenge_method} onChange={(e) => { const o = [...config.per_client]; o[i] = { ...c, challenge_method: e.target.value as "S256" | "plain" }; setConfig({ ...config, per_client: o }); }} className="px-2 py-1 rounded border dark:border-gray-700 dark:bg-gray-900 text-xs"><option value="S256">S256</option><option value="plain">plain</option></select></td></tr>))}</tbody>
            </table>
          </div>
        </>
      )}
