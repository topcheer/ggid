...
export default function TokenLifecycleDashboardPage() {
  ...
  const total = data?.stages.reduce((s: any, d: any) => s + d.count, 0) || 1;
  const maxChurn = Math.max(...(data?.churn_30d.map((d: any) => d.value) || [1]), 1);
  return (
    <div className="space-y-6">
      ...
      {data && (<>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">{t("backend.tokenLifecycle.tokensByStage")}</h3><div className="flex items-center gap-4"><div className="relative w-32 h-32"><svg viewBox="0 0 64 64" className="w-full h-full -rotate-90">{(() => { let offset = 0; return data.stages.map((s: any) => { const pct = s.count / total; const dash = pct * 176; const circle = <circle key={s.stage} cx={32} cy={32} r={28} fill="none" stroke={s.color} strokeWidth={8} strokeDasharray={dash + " 176"} strokeDashoffset={-offset * 176} />; offset += pct; return circle; }); })()}</svg></div><div className="space-y-1">{data.stages.map((s: any) => (<div key={s.stage} className="flex items-center gap-2 text-sm"><span className="w-3 h-3 rounded-full" style={{ background: s.color }} /><span className="capitalize">{s.stage}</span><span className="font-bold ml-auto">{s.count}</span></div>))}</div></div></div>
