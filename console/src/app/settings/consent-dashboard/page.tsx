  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold flex items-center gap-2"><ShieldCheck className="w-6 h-6 text-green-500" /> {t("consentDashboard.title")}</h1><p className="text-sm text-gray-500 mt-1">{t("consentDashboard.subtitle")}</p></div>
        {selected.length > 0 && <button onClick={bulkRevoke} className="px-4 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 flex items-center gap-2"><Ban className="w-4 h-4" /> {t("consentDashboard.revoke")} ({selected.length})</button>}
      </div>

      {data && (
        <>
          <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
            <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium w-8"></th><th className="px-4 py-3 text-left font-medium">{t("consentDashboard.client")}</th><th className="px-4 py-3 text-left font-medium">{t("consentDashboard.users")}</th><th className="px-4 py-3 text-left font-medium">{t("consentDashboard.scopes")}</th><th className="px-4 py-3 text-left font-medium">{t("consentDashboard.consentRate")}</th><th className="px-4 py-3 text-left font-medium">{t("consentDashboard.lastGranted")}</th></tr></thead>
              <tbody className="divide-y dark:divide-gray-800">{data.active_consents.map((c) => (<tr key={c.client_id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3"><input type="checkbox" checked={selected.includes(c.client_id)} onChange={() => toggleSelect(c.client_id)} className="rounded" /></td><td className="px-4 py-3 font-medium">{c.client_name}</td><td className="px-4 py-3 font-bold">{c.user_count}</td><td className="px-4 py-3"><div className="flex flex-wrap gap-1">{c.scopes.slice(0, 3).map((s, i) => <span key={i} className="px-1.5 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-800 font-mono">{s}</span>)}{c.scopes.length > 3 && <span className="text-xs text-gray-400">+{c.scopes.length - 3}</span>}</div></td><td className="px-4 py-3"><span className={"font-bold text-sm " + (c.consent_rate >= 80 ? "text-green-600" : c.consent_rate >= 50 ? "text-yellow-600" : "text-red-600")}>{c.consent_rate.toFixed(0)}%</span></td><td className="px-4 py-3 text-xs text-gray-400">{c.last_granted}</td></tr>))}</tbody>
            </table>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">{t("consentDashboard.revocationTrend30d")}</h3><div className="flex items-end gap-0.5 h-16">{data.revocation_trend.map((t, i) => <div key={i} className="flex-1 bg-red-400 dark:bg-red-500 rounded-t" style={{ height: (t.count / maxRev) * 100 + "%", minHeight: "2px" }} title={t.day + ": " + t.count} />)}</div></div>
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">{t("consentDashboard.pendingExpiry")}</h3><div className="space-y-2">{data.pending_expiry.map((e, i) => <div key={i} className="flex items-center gap-2 text-sm"><Clock className="w-3.5 h-3.5 text-orange-500" /><span className="text-xs flex-1">{e.user} - {e.client_name}</span><span className={"text-xs font-bold " + (e.days_left <= 7 ? "text-red-600" : "text-orange-600")}>{e.days_left}d</span></div>)}{data.pending_expiry.length === 0 && <p className="text-xs text-gray-400">{t("consentDashboard.noneExpiring")}</p>}</div></div>
          </div>
        </>
      )}
      {!data && !loading && <p className="text-sm text-gray-500 text-center py-8">{t("consentDashboard.loading")}</p>}
    </div>
  );
}
