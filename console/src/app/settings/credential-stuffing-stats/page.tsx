...
export default function CredentialStuffingStatsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/auth/credential-stuffing-stats", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Top Source IPs</h3><div className="space-y-1">{data.top_source_ips.map((ip: any, i: number) => (<div key={i} className="flex items-center gap-2"><span className="text-xs font-mono text-gray-500 w-32">{ip.ip}</span><div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-4 overflow-hidden"><div className="h-full bg-red-500 rounded-full" style={{ width: Math.min((ip.attempts / data.top_source_ips[0].attempts) * 100, 100) + "%" }} /></div><span className="text-xs font-bold w-12 text-right">{ip.attempts}</span></div>))}</div></div>
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Top User Agents</h3><div className="space-y-1">{data.top_user_agents.map((ua: any, i: number) => (<div key={i} className="flex items-center gap-2"><span className="text-xs text-gray-500 flex-1 truncate" title={ua.ua}>{ua.ua.length > 50 ? ua.ua.substring(0, 50) + "..." : ua.ua}</span><span className="text-xs font-bold text-red-600">{ua.attempts}</span></div>))}</div></div>
          </div>
        </>
      )}
