  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4 space-y-4 max-w-lg">
        <div><label className="text-sm font-medium">{t("bruteForce.maxAttempts")}</label><div className="flex items-center gap-3 mt-1"><input aria-label="config" type="range" min={3} max={20} value={config.max_attempts} onChange={(e) => setConfig({ ...config, max_attempts: parseInt(e.target.value) })} className="flex-1" /><span className="font-bold text-sm w-10">{config.max_attempts}</span></div></div>
        <div><label className="text-sm font-medium">{t("bruteForce.lockoutDuration")}</label><div className="flex items-center gap-3 mt-1"><input aria-label="config" type="range" min={5} max={120} value={config.lockout_duration_minutes} onChange={(e) => setConfig({ ...config, lockout_duration_minutes: parseInt(e.target.value) })} className="flex-1" /><span className="font-bold text-sm w-12">{config.lockout_duration_minutes}m</span></div></div>
        <div><label className="text-sm font-medium">{t("bruteForce.captchaThreshold")}</label><input aria-label="config" type="number" min={1} max={10} value={config.captcha_threshold} onChange={(e) => setConfig({ ...config, captcha_threshold: parseInt(e.target.value) })} className="w-20 mt-1 px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" /></div>
        <label className="flex items-center gap-2 cursor-pointer"><input aria-label="Config" type="checkbox" checked={config.progressive_delay} onChange={(e) => setConfig({ ...config, progressive_delay: e.target.checked })} className="rounded" /><span className="text-sm">{t("bruteForce.progressiveDelay")}</span></label>
      </div>

      <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Shield className="w-4 h-4 text-gray-400" /> {t("bruteForce.perEndpoint")}</h3><table className="w-full text-sm"><thead><tr><th className="px-4 py-2 text-left font-medium">{t("bruteForce.endpoint")}</th><th className="px-4 py-2 text-left font-medium">{t("bruteForce.maxAttempts")}</th><th className="px-4 py-2 text-left font-medium">{t("bruteForce.lockoutMin")}</th></tr></thead><tbody className="divide-y dark:divide-gray-800">{config.endpoint_overrides.map((e: any, i: number) => (<tr key={i}><td className="px-4 py-2 font-mono text-xs">{e.endpoint}</td><td className="px-4 py-2"><input aria-label="e" type="number" value={e.max_attempts} onChange={(ev) => { const o = [...config.endpoint_overrides]; o[i] = { ...e, max_attempts: parseInt(ev.target.value) }; setConfig({ ...config, endpoint_overrides: o }); }} className="w-16 px-2 py-1 rounded border dark:border-gray-700 dark:bg-gray-900 text-sm" /></td><td className="px-4 py-2"><input type="number" value={e.lockout_minutes} onChange={(ev) => { const o = [...config.endpoint_overrides]; o[i] = { ...e, lockout_minutes: parseInt(ev.target.value) }; setConfig({ ...config, endpoint_overrides: o }); }} className="w-16 px-2 py-1 rounded border dark:border-gray-700 dark:bg-gray-900 text-sm" /></td></tr>))}</tbody></table></div>

      {lockouts.length > 0 && <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3 text-red-600">{t("bruteForce.currentLockouts")} ({lockouts.length})</h3><div className="space-y-1">{lockouts.map((l: any, i: number) => (<div key={i} className="flex items-center gap-2 text-sm"><span className="font-mono text-xs">{l.ip}</span><span className="text-xs">{l.username}</span><span className="text-xs text-gray-400 ml-auto">unlock in {l.minutes_remaining}m</span></div>))}</div></div>}
    </div>
  );
}
