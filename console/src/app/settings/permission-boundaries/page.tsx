...
export default function PermissionBoundariesPage() {
  ...
  const saveEdit = async () => {
    ...
    setSaving(true);
    try {
      await fetch(`/api/v1/policy/permission-boundaries/${editForm.id}`, {
        method: "PUT",
                ...
                <div>
                  ...
                  <div className="mt-1 space-y-1">
                    {editForm.denied_actions.map((a, i) => (
                      <div key={i} className="flex items-center gap-2"><span className="flex-1 px-3 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-800 text-sm font-mono">{a}</span><button onClick={() => setEditForm({ ...editForm, denied_actions: editForm.denied_actions.filter((_, idx) => idx !== i) })} className="p-1 text-red-400"><Trash2 className="w-4 h-4" /></button></div>
                    ))}
                    <div className="flex items-center gap-2"><input type="text" value={newDenied} onChange={(e) => setNewDenied(e.target.value)} placeholder="delete:organizations" className="flex-1 px-3 py-1.5 rounded border dark:border-gray-700 dark:bg-gray-800 text-sm font-mono" /><button onClick={() => { if (newDenied.trim()) { setEditForm({ ...editForm, denied_actions: [...editForm.denied_actions, newDenied.trim()] }); setNewDenied(""); } }} className="p-1.5 rounded bg-blue-600 text-white"><Plus className="w-4 h-4" /></button></div>
                  </div>
                </div>
              ...
              <div className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {b.violation_count > 0 ? (
                      <button onClick={() => { setShowViolations(showViolations === b.id ? null : b.id); if (showViolations !== b.id) fetchViolations(b.id); }} className="px-2 py-0.5 rounded text-xs bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400 hover:underline">{b.violation_count} violations</button>
                    ) : (
                      <span className="flex items-center gap-1 text-xs text-green-600"><CheckCircle2 className="w-3 h-3" /> No violations</span>
                    )}
                  </div>
                  <button onClick={() => startEdit(b)} className="text-blue-600 hover:underline text-sm font-medium flex items-center gap-1"><Edit3 className="w-3 h-3" /> Edit</button>
                </div>
