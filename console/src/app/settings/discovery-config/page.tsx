...
export default function DiscoveryConfigPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await fetch("/api/v1/oauth/discovery-config", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); if (res.ok) setData(await res.json()); }
    catch { /* noop */ }
  ...
  const refreshJwks = async () => {
    setRefreshing(true);
    try { await fetch("/api/v1/oauth/discovery-config/jwks-refresh", { method: "POST", headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); fetchData(); }
    catch { /* noop */ }
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">{t("backend.discoveryConfig.supportedScopes")}</h3><div className="space-y-1">{data.supported_scopes.map((s: any) => <div key={s.name} className="flex items-center gap-2"><span className="font-mono text-xs font-medium">{s.name}</span><span className="text-xs text-gray-400">{s.description}</span></div>)}</div></div>
            <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">{t("backend.discoveryConfig.supportedGrants")}</h3><div className="flex flex-wrap gap-2">{data.supported_grants.map((g: any) => <span key={g} className="px-2 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-800 font-mono">{g}</span>)}</div><h3 className="text-sm font-semibold mt-4 mb-3">{t("backend.discoveryConfig.signingAlgorithms")}</h3><div className="flex flex-wrap gap-2">{data.signing_algs.map((a: any) => <span key={a} className="px-2 py-0.5 rounded text-xs bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 font-mono">{a}</span>)}</div></div>
          </div>
