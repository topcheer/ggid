    ...
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div><label className="block text-sm font-medium mb-1">{t("big1.deviceAuthorizationFlowConfig.codeLifetimeS")}</label><input type="number" value={form.device_code_lifetime} onChange={(e) => setForm({ ...form, device_code_lifetime: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div>
          <div><label className="block text-sm font-medium mb-1">{t("big1.deviceAuthorizationFlowConfig.pollingIntervalS")}</label><input type="number" value={form.polling_interval} onChange={(e) => setForm({ ...form, polling_interval: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div>
          <div><label className="block text-sm font-medium mb-1">{t("big1.deviceAuthorizationFlowConfig.userCodeFormat")}</label><select value={form.user_code_format} onChange={(e) => setForm({ ...form, user_code_format: e.target.value as DeviceAuthorizationFlowConfig["user_code_format"] })} className="border rounded px-3 py-2"><option value="numeric">{t("big1.deviceAuthorizationFlowConfig.numeric")}</option><option value="alphanumeric">{t("big1.deviceAuthorizationFlowConfig.alphanumeric")}</option></select></div>
        </div>
        <div className="flex items-center gap-3"><input type="checkbox" checked={form.qr_code_enabled} onChange={(e) => setForm({ ...form, qr_code_enabled: e.target.checked })} className="w-4 h-4" /><label>{t("big1.deviceAuthorizationFlowConfig.qrCodeEnabled")}</label></div>
      </div>
      ...
      <button onClick={handleSave} disabled={saving} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">{saving ? t("big1.deviceAuthorizationFlowConfig.saving") : t("big1.deviceAuthorizationFlowConfig.saveChanges")}</button>
    </div>
  );
}
