...
export default function BatchSimulatePage() {
  ...
  const remove = (val: string, list: string[], setter: (v: string[]) => void) => setter(list.filter((x: any) => x !== val));

  const simulate = useCallback(async () => {
    if (subjects.length === 0 || resources.length === 0 || actions.length === 0) return;
    setLoading(true);
    try {
      const res = await fetch("/api/v1/policy/batch-simulate", { method: "POST", headers: { ...authHeader(), "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" }, body: JSON.stringify({ subjects, resources, actions }) });
        ...
        <>
          ...
          <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
            <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Subject</th><th className="px-4 py-3 text-left font-medium">Resource</th><th className="px-4 py-3 text-left font-medium">Action</th><th className="px-4 py-3 text-left font-medium">Decision</th><th className="px-4 py-3 text-left font-medium">Policy</th></tr></thead>
              <tbody className="divide-y dark:divide-gray-800">{data.results.map((r: any, i: number) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3 font-mono text-xs">{r.subject}</td><td className="px-4 py-3 font-mono text-xs">{r.resource}</td><td className="px-4 py-3 text-xs">{r.action}</td><td className="px-4 py-3">{r.decision === "allow" ? <span className="flex items-center gap-1 text-xs text-green-600"><Check className="w-3 h-3" /> Allow</span> : <span className="flex items-center gap-1 text-xs text-red-600"><Ban className="w-3 h-3" /> Deny</span>}</td><td className="px-4 py-3 font-mono text-xs text-gray-500">{r.matched_policy || "-"}</td></tr>))}</tbody>
            </table>
          </div>
        </>
      )}
