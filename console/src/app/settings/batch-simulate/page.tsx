...
export default function BatchSimulatePage() {
  ...
  const Chip = ({ val, onRemove }: { val: string; onRemove: () => void }) => (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 font-mono">{val}<button onClick={onRemove}><X className="w-3 h-3" /></button></span>
  );
