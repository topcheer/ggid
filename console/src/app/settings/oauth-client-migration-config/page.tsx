...
export default function OAuthClientMigrationConfigPage() {
  ...
  if (loading && !form) return <div className="p-8">{t("oauthMigration.loading")}</div>;
  if (error) return <div className="p-8 text-red-600">Error: {error}</div>;
  if (!form) return <div className="p-8">{t("oauthMigration.noData")}</div>;

  return (
    <div className="p-8 space-y-6 max-w-4xl">
      <h1 className="text-2xl font-bold">{t("oauthMigration.title")}</h1>
      <p className="text-gray-600">{t("oauthMigration.subtitle")}</p>
...
// @ts-ignore
      ...
      <div className="bg-white rounded-lg p-6 shadow">
        <h2 className="text-lg font-semibold mb-4">{t("oauthMigration.validationReport")}</h2>
        <div className="grid grid-cols-4 gap-4">
          <div className="text-center"><div className="text-2xl font-bold">{form.validation_report.total_clients}</div><div className="text-xs text-gray-500">{t("oauthMigration.totalClients")}</div></div>
          <div className="text-center"><div className="text-2xl font-bold text-green-600">{form.validation_report.mapped}</div><div className="text-xs text-gray-500">{t("oauthMigration.mapped")}</div></div>
          ...
          <div className="text-center"><div className="text-2xl font-bold text-red-600">{form.validation_report.errors}</div><div className="text-xs text-gray-500">{t("oauthMigration.errors")}</div></div>
        </div>
      </div>

      <div className="bg-white rounded-lg p-6 shadow">
        <h2 className="text-lg font-semibold mb-4">{t("oauthMigration.phasedCutover")}</h2>
