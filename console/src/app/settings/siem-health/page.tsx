...
export default function SIEMHealthPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/audit/siem-health", { headers: { ...authHeader(), "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  const overall = destinations.some((d: any) => d.status === "down") ? "critical" : destinations.some((d: any) => d.status === "degraded") ? "degraded" : destinations.length > 0 ? "healthy" : "unknown";
