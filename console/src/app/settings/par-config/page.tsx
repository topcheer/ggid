...
export default function ParConfigPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-lg border dark:border-gray-800"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Client</th><th className="px-4 py-3 text-left font-medium">PAR Required</th></tr></thead><tbody className="divide-y dark:divide-gray-800">{config.per_client.map((c, i) => (<tr key={c.client_id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3"><span className="font-medium">{c.client_name}</span></td><td className="px-4 py-3"><label className="flex items-center gap-2"><input aria-label="C" type="checkbox" checked={c.required} onChange={(e) => { const o = [...config.per_client]; o[i] = { ...c, required: e.target.checked }; setConfig({ ...config, per_client: o }); }} className="rounded" /><span className="text-xs">{c.required ? "Yes" : "No"}</span></label></td></tr>))}</tbody></table></div>
    </div>
  );
}
