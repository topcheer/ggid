...
export default function ParConfigPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {config.exempted_clients.length > 0 && <div className="rounded-lg border border-yellow-300 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 p-3"><span className="text-sm font-medium text-yellow-700 dark:text-yellow-400">Exempted: </span>{config.exempted_clients.map((c) => <span key={c} className="font-mono text-xs mr-2">{c}</span>)}</div>}
      <div className="overflow-x-auto rounded-lg border dark:border-gray-800"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Client</th><th className="px-4 py-3 text-left font-medium">PAR Required</th></tr></thead><tbody className="divide-y dark:divide-gray-800">{config.per_client.map((c, i) => (<tr key={c.client_id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3"><span className="font-medium">{c.client_name}</span></td><td className="px-4 py-3"><label className="flex items-center gap-2"><input type="checkbox" checked={c.required} onChange={(e) => { const o = [...config.per_client]; o[i] = { ...c, required: e.target.checked }; setConfig({ ...config, per_client: o }); }} className="rounded" /><span className="text-xs">{c.required ? "Yes" : "No"}</span></label></td></tr>))}</tbody></table></div>
    </div>
  );
}
