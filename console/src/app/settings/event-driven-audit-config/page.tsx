  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-4">
        ...
        <div><label className="block text-sm font-medium mb-1">Backpressure Strategy</label><select value={form.backpressure_strategy} onChange={(e) => setForm({ ...form, backpressure_strategy: e.target.value as EventDrivenAuditConfig["backpressure_strategy"] })} className="border rounded px-3 py-2"><option value="block">Block</option><option value="drop">Drop</option><option value="buffer">Buffer</option></select></div>
        <div className="flex items-center gap-3"><input type="checkbox" checked={form.replay_enabled} onChange={(e) => setForm({ ...form, replay_enabled: e.target.checked })} className="w-4 h-4" /><label>Replay Enabled</label></div>
      </div>
