    ...
    <div className="space-y-6">
      ...
      {data && (<>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-lg border dark:border-gray-800 p-4 flex items-center gap-4"><div className="relative w-24 h-24"><svg viewBox="0 0 64 64" className="w-full h-full"><circle cx={32} cy={32} r={28} fill="none" stroke="currentColor" strokeWidth={6} className="text-gray-200 dark:text-gray-800" /><circle cx={32} cy={32} r={28} fill="none" stroke={scoreColor} strokeWidth={6} strokeDasharray={(data.integrity_score / 100) * 176 + " 176"} strokeLinecap="round" transform="rotate(-90 32 32)" /></svg><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-2xl font-bold" style={{ color: scoreColor }}>{data.integrity_score}%</span><span className="text-[10px] text-gray-400">integrity</span></div></div><div><span className="text-sm text-gray-500">Total Blocks</span><p className="text-xl font-bold">{data.total_blocks.toLocaleString()}</p></div></div>
          <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-2">Verification Log</h3><div className="space-y-1 max-h-32 overflow-y-auto">{data.verify_log.map((v, i) => (<div key={i} className="text-xs flex items-center gap-2"><span className={v.status === "ok" ? "text-green-600" : "text-red-600"}>{v.status}</span><span className="text-gray-400">{v.blocks_checked} blocks</span><span className="text-gray-400 ml-auto">{v.timestamp}</span></div>))}</div></div>
        </div>
        {data.tamper_alerts.length > 0 && <div className="rounded-lg border border-red-300 dark:border-red-800 p-4"><h3 className="text-sm font-semibold text-red-700 dark:text-red-400 mb-2 flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> Tamper Alerts ({data.tamper_alerts.length})</h3><div className="space-y-2">{data.tamper_alerts.map((a, i) => (<div key={i} className="rounded border dark:border-gray-800 p-2 text-xs"><div className="flex items-center justify-between"><span className="font-bold">Block #{a.block_num}</span><span className="text-gray-400">{a.detected_at}</span></div><div className="mt-1 font-mono text-gray-500"><span className="text-red-500">Expected: {a.expected_hash}</span></div><div className="font-mono text-gray-500"><span className="text-red-500">Actual: {a.actual_hash}</span></div></div>))}</div></div>}
      </>)}
