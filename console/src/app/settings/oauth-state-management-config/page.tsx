    ...
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">State Length (bytes)</label><input type="number" value={form.state_length_bytes} onChange={(e) => setForm({ ...form, state_length_bytes: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div>
          <div><label className="block text-sm font-medium mb-1">State TTL (seconds)</label><input type="number" value={form.state_ttl_seconds} onChange={(e) => setForm({ ...form, state_ttl_seconds: parseInt(e.target.value) || 0 })} className="border rounded px-3 py-2 w-full" /></div>
        </div>
        <div><label className="block text-sm font-medium mb-1">Binding Method</label><select value={form.binding_method} onChange={(e) => setForm({ ...form, binding_method: e.target.value as OAuthStateManagementConfig["binding_method"] })} className="border rounded px-3 py-2"><option value="session">Session</option><option value="cookie">Cookie</option><option value="jwt">JWT</option></select></div>
        <div><label className="block text-sm font-medium mb-1">Validation Strictness</label><select value={form.validation_strictness} onChange={(e) => setForm({ ...form, validation_strictness: e.target.value as OAuthStateManagementConfig["validation_strictness"] })} className="border rounded px-3 py-2"><option value="strict">Strict</option><option value="standard">Standard</option><option value="lenient">Lenient</option></select></div>
      </div>
