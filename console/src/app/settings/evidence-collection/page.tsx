...
export default function EvidenceCollectionPage() {
  ...
  const collected = data?.controls.filter((c: any) => c.collection_status === "collected").length || 0;
  const pending = data?.controls.filter((c: any) => c.collection_status === "pending").length || 0;
  const overdue = data?.controls.filter((c: any) => c.collection_status === "overdue").length || 0;
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3"><select aria-label="Framework" value={framework} onChange={(e) => setFramework(e.target.value)} className="px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm">{frameworks.map((f: any) => <option key={f} value={f}>{f}</option>)}</select>{data && <span className="text-sm text-gray-500">{collected}/{data.controls.length} collected, {pending} pending, {overdue} overdue</span>}</div>
