...
export default function PasswordPolicyConfigPage() {
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/v1/auth/password-policy-config", { headers: { ...authHeader(), "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  const save = async () => { setSaving(true); try { await fetch("/api/v1/auth/password-policy-config", { method: "PUT", headers: { ...authHeader(), "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" }, body: JSON.stringify(config) }); } catch { /* noop */ } finally { setSaving(false); } };
