...
const permLabels: Record<string, string> = { view_users: "View Users", manage_users: "Manage Users", view_policies: "View Policies", manage_policies: "Manage Policies", view_audit: "View Audit", manage_audit: "Manage Audit", view_oauth: "View OAuth", manage_oauth: "Manage OAuth" };
...
export default function AdminSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold flex items-center gap-2"><Shield className="w-6 h-6 text-red-500" /> Admin Settings</h1><p className="text-sm text-gray-500 mt-1">Manage super admins, permissions, and restricted actions.</p></div>

      <div className="flex items-center justify-between rounded-lg border dark:border-gray-800 p-4"><div className="flex items-center gap-3"><ShieldCheck className="w-8 h-8 text-blue-500" /><div><span className="text-sm text-gray-500">Require MFA for Admins</span><p className="text-xs text-gray-400">All admin actions require multi-factor authentication</p></div></div><label className="relative inline-flex items-center cursor-pointer"><input type="checkbox" checked={config.require_mfa} onChange={(e) => setConfig({ ...config, require_mfa: e.target.checked })} className="sr-only peer" /><div className="w-11 h-6 bg-gray-200 rounded-full peer dark:bg-gray-700 peer-checked:bg-blue-600 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5" /></label></div>

      <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Super Admins ({config.super_admins.length})</h3><div className="space-y-1">{config.super_admins.map((a) => (<div key={a.user_id} className="flex items-center justify-between text-sm py-1"><div><span className="font-medium">{a.username}</span><p className="text-xs text-gray-400 font-mono">{a.user_id}</p></div><div className="text-right"><span className="text-xs text-gray-500">Added {a.added_at}</span><p className="text-xs text-gray-400">by {a.added_by}</p></div></div>))}</div></div>

      <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Admin Permissions Matrix</h3><div className="grid grid-cols-2 md:grid-cols-4 gap-2">{Object.entries(permLabels).map(([key, label]) => (<label key={key} className="flex items-center gap-2 text-sm rounded-lg border dark:border-gray-700 p-2"><input type="checkbox" checked={config.permissions[key] || false} onChange={(e) => setConfig({ ...config, permissions: { ...config.permissions, [key]: e.target.checked } })} className="rounded" /> {label}</label>))}</div></div>

      {config.restricted_actions.length > 0 && (<div className="rounded-lg border border-red-200 dark:border-red-900 p-4"><h3 className="text-sm font-semibold mb-2 text-red-600">Restricted Actions</h3><div className="space-y-1">{config.restricted_actions.map((a, i) => (<div key={i} className="text-sm font-mono text-xs text-red-600">{a}</div>))}</div></div>)}

      <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Activity className="w-4 h-4 text-gray-400" /> Admin Activity Log</h3><div className="space-y-1">{config.activity_log.slice(0, 10).map((a) => (<div key={a.id} className="flex items-center justify-between text-sm py-1"><div><span className="font-medium">{a.admin}</span><span className="text-gray-500"> {a.action}</span><span className="text-xs text-gray-400"> {a.target}</span></div><span className="text-xs text-gray-400">{a.timestamp}</span></div>))}{config.activity_log.length === 0 && <p className="text-xs text-gray-500">No activity.</p>}</div></div>
    </div>
  );
}
