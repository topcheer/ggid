...
export default function PasswordResetFlowPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">Reset Methods</h3><div className="grid grid-cols-2 gap-3">{Object.entries(config.methods).map(([key, val]) => { const Icon = methodIcons[key] || KeyRound; return (<label key={key} className="flex items-center gap-2 text-sm rounded-lg border dark:border-gray-700 p-3"><input type="checkbox" checked={val} onChange={(e) => setConfig({ ...config, methods: { ...config.methods, [key]: e.target.checked } })} className="rounded" /> <Icon className="w-4 h-4 text-gray-400" /> {key.replace(/_/g, " ")}</label>); })}</div></div>

      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-lg border dark:border-gray-800 p-4"><label className="text-sm font-medium">Token Expiry (minutes)</label><input type="number" value={config.token_expiry_minutes} onChange={(e) => setConfig({ ...config, token_expiry_minutes: parseInt(e.target.value) || 0 })} className="w-full mt-1 px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" /></div>
        <div className="rounded-lg border dark:border-gray-800 p-4"><label className="text-sm font-medium">Reset After Failed Attempts</label><input type="number" value={config.reset_after_failed_attempts} onChange={(e) => setConfig({ ...config, reset_after_failed_attempts: parseInt(e.target.value) || 0 })} className="w-full mt-1 px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" /></div>
      </div>

      <div className="flex items-center gap-6"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={config.require_mfa} onChange={(e) => setConfig({ ...config, require_mfa: e.target.checked })} className="rounded" /> Require MFA for reset</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={config.notify_on_reset} onChange={(e) => setConfig({ ...config, notify_on_reset: e.target.checked })} className="rounded" /> Notify user on reset</label></div>
