...
export default function IdentityAttributeSchemaConfigPage() {
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-lg p-6 shadow space-y-3"><h2 className="text-lg font-semibold">{t("idAttributeSchema.standardAttrs")}</h2><div className="text-sm text-gray-600">{form.standard_attributes.join(", ")}</div><div className="flex items-center gap-3"><input type="checkbox" checked={form.schema_extension} onChange={(e) => setForm({ ...form, schema_extension: e.target.checked })} className="w-4 h-4" /><label>{t("idAttributeSchema.allowExtension")}</label></div></div>
