...
export default function IdentityAttributeSchemaConfigPage() {
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      <h1 className="text-2xl font-bold">Identity Attribute Schema Configuration</h1>
      <p className="text-gray-600">Configure standard and custom identity attributes.</p>
      <div className="bg-white rounded-lg p-6 shadow space-y-3"><h2 className="text-lg font-semibold">Standard Attributes</h2><div className="text-sm text-gray-600">{form.standard_attributes.join(", ")}</div><div className="flex items-center gap-3"><input type="checkbox" checked={form.schema_extension} onChange={(e) => setForm({ ...form, schema_extension: e.target.checked })} className="w-4 h-4" /><label>Allow Schema Extension</label></div></div>
      <div className="bg-white rounded-lg p-6 shadow"><h2 className="text-lg font-semibold mb-4">Custom Attributes</h2><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="py-2">Name</th><th>Type</th><th>Multi</th><th>Required</th><th>Validation</th><th>Privacy</th></tr></thead><tbody>{form.custom_attributes.map((a: CustomAttribute, i: number) => (<tr key={i} className="border-b"><td className="py-2 font-medium">{a.name}</td><td>{a.type}</td><td>{a.multi_valued ? "Yes" : "No"}</td><td>{a.required ? "Yes" : "No"}</td><td className="text-xs font-mono">{a.validation_rule}</td><td><span className={`px-2 py-1 rounded text-xs ${privColors[a.privacy_classification] || ""}`}>{a.privacy_classification}</span></td></tr>))}</tbody></table></div>
      <div className="bg-white rounded-lg p-6 shadow"><h2 className="text-lg font-semibold mb-4">Per-Attribute Masking</h2><div className="space-y-1">{form.per_attribute_masking.map((m: { attribute: string; masked: boolean }, i: number) => (<div key={i} className="flex items-center justify-between border-b py-1"><span className="text-sm">{m.attribute}</span><span className={`px-2 py-1 rounded text-xs ${m.masked ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-500"}`}>{m.masked ? "Masked" : "Visible"}</span></div>))}</div></div>
