...
export default function TokenFamilyPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/oauth/token-family", { headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  const revokeFamily = async (familyId: string) => {
    try { await fetch("/api/v1/oauth/token-family/" + familyId + "/revoke", { method: "POST", headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } }); fetchData(); }
    catch { /* noop */ }
  };
