...
export default function ComplianceTimelinePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">{frameworks.map((f: any) => <button key={f} onClick={() => setTab(f)} className={"px-4 py-2 rounded-lg text-sm font-medium " + (tab === f ? "bg-green-600 text-white" : "border dark:border-gray-700")}>{f}</button>)}</div>
      {data && (<>
        <div className="relative pl-8"><div className="absolute left-3 top-0 bottom-0 w-px bg-gray-200 dark:bg-gray-800" /><div className="space-y-4">{data.milestones.map((m: any) => (<div key={m.id} className="relative"><div className={"absolute -left-5 w-4 h-4 rounded-full border-2 border-white " + statusBg[m.status]} /><div className="rounded-lg border dark:border-gray-800 p-3 ml-2"><div className="flex items-center justify-between"><span className="font-medium text-sm">{m.name}</span><span className={"text-xs font-medium " + statusColors[m.status]}>{m.status.replace("_", " ")}</span></div><div className="flex items-center gap-3 mt-1 text-xs text-gray-500"><span className="flex items-center gap-1"><Clock className="w-3 h-3" />{m.due_date}</span><span>Owner: {m.responsible_party}</span></div></div></div>))}</div></div>
      </>)}
