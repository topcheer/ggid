        ...
        <div className="space-y-3">{clients.map((c) => (
          <div key={c.id} className={`${cardCls} ${c.status !== "active" ? "border-l-4 border-l-yellow-400" : ""}`}>
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2"><button onClick={() => setPreview(c)} className="text-xs text-indigo-600 hover:underline"><Eye className="inline h-3 w-3" /> Warning</button><select value={c.status} onChange={(e) => handleUpdateStatus(c.id, e.target.value as DeprecationStatus["status"])} className="rounded border border-gray-300 px-2 py-1 text-xs dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"><option value="active">Active</option><option value="deprecated">Deprecated</option><option value="sunset">Sunset</option><option value="retired">Retired</option></select></div>
            </div>
          </div>
        ))}</div>
      )}
      ...
      {preview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setPreview(null)}>
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between"><h3 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white"><AlertTriangle className="h-5 w-5 text-yellow-600" /> Deprecation Warning Preview</h3><button onClick={() => setPreview(null)}><X className="h-5 w-5 text-gray-400" /></button></div>
