...
export default function GroupPermissionTreePage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/policy/group-permission-tree", { headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border dark:border-gray-800 p-4"><div className="space-y-0.5">{tree.map((n) => renderNode(n, 0))}{tree.length === 0 && !loading && <p className="text-sm text-gray-500 text-center py-4">No data.</p>}</div></div>
    </div>
  );
}
