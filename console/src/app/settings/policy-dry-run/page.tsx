...
export default function PolicyDryRunPage() {
  ...
  const evaluate = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/v1/policy/dry-run", { method: "POST", headers: { ...authHeader(), "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" }, body: JSON.stringify({ policy_id: policyId, subject, resource, action: action || "access" }) });
        ...
        return (
          <>
            ...
            {result.matched_rules.length > 0 && (
              <div className="rounded-lg border dark:border-gray-800 p-4"><h3 className="text-sm font-semibold mb-3">{t("policyDryRun.matchedRules")}</h3><div className="space-y-2">{result.matched_rules.map((r: any) => (<div key={r.rule_id} className="flex items-center gap-2"><span className="font-mono text-xs text-gray-500">{r.rule_id}</span><span className="text-sm flex-1">{r.rule_name}</span><span className={"px-2 py-0.5 rounded text-xs " + (r.effect === "allow" ? "bg-green-100 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 dark:bg-red-900/30 dark:text-red-400")}>{r.effect}</span></div>))}</div></div>
            )}
