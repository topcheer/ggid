            ...
            <>
              ...
              <div className="overflow-x-auto rounded-lg border dark:border-gray-800">
                <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900/50"><tr><th className="px-4 py-3 text-left font-medium">Type</th><th className="px-4 py-3 text-left font-medium">Name</th><th className="px-4 py-3 text-left font-medium">Current</th><th className="px-4 py-3 text-left font-medium">Projected</th><th className="px-4 py-3 text-left font-medium">Risk</th></tr></thead>
                  <tbody className="divide-y dark:divide-gray-800">{result.items.map((item: any) => (<tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30"><td className="px-4 py-3"><span className="text-xs font-medium uppercase text-gray-500">{item.type}</span></td><td className="px-4 py-3 font-medium">{item.name}</td><td className="px-4 py-3 text-xs text-gray-500">{item.current_access}</td><td className="px-4 py-3 text-xs font-medium">{item.projected_access}</td><td className="px-4 py-3"><span className={`px-2 py-0.5 rounded text-xs ${riskColors[item.risk_level]}`}>{item.risk_level}</span></td></tr>))}</tbody>
                </table>
              </div>
            </>
          )}
