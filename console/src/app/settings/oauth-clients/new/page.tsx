      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("oauthWizard.title")}</h1>
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                ...
                {appTypes.map((a) => {
                  ...
                  return (
                    <button key={a.id} onClick={() => { setAppType(a.id); if (a.id === "spa") setAuthMethod("pkce"); if (a.id === "m2m") setAuthMethod("client_secret"); }}
                      ...
                      <div>
                        <span className="text-sm font-bold text-gray-900 dark:text-white">{t(`oauthWizard.appType.${a.labelKey}`)}</span>
                ...
                return (
                  <button key={m.id} onClick={() => !disabled && setAuthMethod(m.id)} disabled={disabled}
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-gray-900 dark:text-white">{t(`oauthWizard.authMethod.${m.labelKey}`)}</span>
          ...
          {step === 2 && (
            <div className="space-y-3">
                ...
                {standardScopes.map((s) => {
                  ...
                  return (
                    <label key={s.id} className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${selected ? "border-blue-500 bg-blue-50 dark:bg-blue-950/20" : "border-gray-200 dark:border-gray-700 hover:border-gray-300"}`}>
                      ...
                      <div className="flex-1">
                        <span className="text-sm font-medium text-gray-900 dark:text-white">{t(`oauthWizard.scopes.${s.labelKey}`)}{s.required && <span className="text-xs text-gray-400 ml-1">(required)</span>}</span>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{t("oauthWizard.scopes.custom")}</label>
          ...
          {step === 3 && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">{t("oauthWizard.confirm.title")}</h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{t("oauthWizard.confirm.clientName")}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{t("oauthWizard.confirm.redirectUris")}</label>
                ...
                <div className="grid grid-cols-2 gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                  <div><span className="text-xs text-gray-400">{t("oauthWizard.confirm.appTypeLabel")}</span><p className="text-sm font-medium text-gray-900 dark:text-white capitalize">{t(`oauthWizard.appType.${appType}`)}</p></div>
                  <div><span className="text-xs text-gray-400">{t("oauthWizard.confirm.authMethodLabel")}</span><p className="text-sm font-medium text-gray-900 dark:text-white">{authMethod ? t(`oauthWizard.authMethod.${authMethod === "client_secret" ? "clientSecret" : authMethod === "pkce" ? "pkce" : "mtls"}`) : "—"}</p></div>
                </div>
              </div>
          ...
          {step === 4 && created && (
            <div className="text-center space-y-4">
              ...
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t("oauthWizard.confirm.created")}</h3>
              ...
              <div className="text-left space-y-3">
                ...
                <button onClick={copyAll} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-lg text-sm hover:bg-gray-200">
  ...
  -d "client_secret=${created.client_secret}"`}</pre>
          ...
          {step < 3 && (
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200 dark:border-gray-800">
              {step > 0 ? (
                <button onClick={() => setStep(step - 1)} className="flex items-center gap-1.5 px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-lg text-sm font-medium">
