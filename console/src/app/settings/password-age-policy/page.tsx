...
export default function PasswordAgePolicyPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/v1/auth/password-age-policy", { headers: { "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" } });
  ...
  const save = async () => {
    if (!policy) return;
    setSaving(true);
    try { await fetch("/api/v1/auth/password-age-policy", { method: "PUT", headers: { "Content-Type": "application/json", "X-Tenant-ID": "00000000-0000-0000-0000-000000000001" }, body: JSON.stringify(policy) }); setSaved(true); setTimeout(() => setSaved(false), 2000); }
    catch { /* noop */ }
      ...
      {policy && (
        <>
          <div className="rounded-lg border dark:border-gray-800 p-6 space-y-4 max-w-lg">
            <div><label className="text-sm font-medium">Max Age (days)</label><input type="number" min={1} value={policy.max_age_days} onChange={(e) => setPolicy({ ...policy, max_age_days: parseInt(e.target.value) || 0 })} className="w-full mt-1 px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" /></div>
            <div><label className="text-sm font-medium">Expiry Warning (days before)</label><input type="number" min={1} value={policy.expiry_warning_days} onChange={(e) => setPolicy({ ...policy, expiry_warning_days: parseInt(e.target.value) || 0 })} className="w-full mt-1 px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" /></div>
            ...
            <div className="flex items-center gap-2"><button onClick={save} disabled={saving} className="px-4 py-2 rounded-lg bg-orange-600 text-white text-sm font-medium hover:bg-orange-700 disabled:opacity-50 flex items-center gap-2"><Save className="w-4 h-4" /> {saving ? "Saving..." : "Save"}</button>{saved && <span className="text-sm text-green-600">Saved!</span>}</div>
          </div>
