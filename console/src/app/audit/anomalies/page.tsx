          ...
          {actionModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setActionModal(null)}>
              <div role="dialog" aria-modal="true" className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
                ...
                <button aria-label="action" onClick={handleAction} disabled={actioning === actionModal.anomaly.id} className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-orange-600 py-2 text-sm font-medium text-white capitalize hover:bg-orange-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500">{actioning === actionModal.anomaly.id ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{actionModal.type}</button>
              </div>
            </div>
          )}
        </>
