    ...
    <div>
              ...
              <>
                ...
                <div className="absolute right-0 z-20 mt-1 w-44 rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
                  <button
                    ...
                    className="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                   aria-label="FileText">
      ...
      {showCsvImport && (
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={closeCsvImport} className="text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Close">
              <X className="h-5 w-5" />
            </button>
          </div>
      ...
      {showImport && (
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          />
          <button onClick={handleImportCSV} disabled={!importText.trim()} className="mt-3 rounded-lg bg-brand-600 px-4 py-2 text-sm text-white hover:bg-brand-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="trim">
            {t("users.importUsers")}
          </button>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="mt-4 flex gap-2">
            <button aria-label="action" type="submit" disabled={creating} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500">{creating ? "Creating..." : t("common.create")}</button>
            <button type="button" onClick={() => { setShowCreate(false); setFormError(""); setFieldErrors({}); }} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">{t("common.cancel")}</button>
          </div>
        </form>
      )}
      {formSuccess && (
                  ...
                  <tr key={user.id} className={`hover:bg-gray-50 ${selected.has(user.id) ? "bg-blue-50/40" : ""} focus:outline-none focus:ring-2 focus:ring-blue-500} focus:outline-none focus:ring-2 focus:ring-blue-500}`}>
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/users/${user.id}`} className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium hover:text-brand-600 focus:outline-none focus:ring-2 focus:ring-blue-500">{user.username}</p>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-1">
                        {user.status === "active" ? (
                          <button onClick={() => handleLock(user.id, user.status)} title={t("users.lock")} className="rounded p-1.5 text-gray-400 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <Lock className="h-4 w-4" />
                          </button>
                        ) : (
                          <button onClick={() => handleLock(user.id, user.status)} title={t("users.unlock")} className="rounded p-1.5 text-gray-400 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <Unlock className="h-4 w-4" />
                          </button>
                        )}
                        <button onClick={() => handleDelete(user.id, user.username)} aria-label={`Delete user ${user.username}`} title={t("common.delete")} className="rounded p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeleteTarget(null)}>
          <div className="rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900 max-w-md" onClick={e => e.stopPropagation()}>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => setDeleteTarget(null)} className="rounded-lg border border-gray-300 px-4 py-2 text-sm dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500">
                Cancel
              </button>
              <button
                onClick={async () => { await doDelete(deleteTarget.ids, deleteTarget.label); setDeleteTarget(null); }}
                className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                Delete
              </button>
            </div>
