...
import { Network, Search, ChevronDown, ChevronRight, Building2, User as UserIcon, Briefcase } from "lucide-react";
  ...
  return (
    <div className="relative">
      <div className={`flex items-center gap-2 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900/30 ${isMatch ? "bg-yellow-50 dark:bg-yellow-900/20" : ""}`} style={{ paddingLeft: `${depth * 24 + 8}px` }}>
        {hasChildren ? (
          <button onClick={() => toggleNode(node.id)} className="p-0.5 rounded hover:bg-gray-200 dark:hover:bg-gray-800">
...
export default function OrgChartPage() {
  ...
  useEffect(() => {
    if (selectedOrg) fetchTree(selectedOrg);
  }, [selectedOrg, fetchTree]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <select value={selectedOrg} onChange={(e) => setSelectedOrg(e.target.value)} className="px-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm">
        ...
        <div className="relative flex-1 max-w-xs ml-auto">
          ...
          <input type="text" placeholder="Search person..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-900 text-sm" />
        </div>
        {tree && (
          <div className="flex items-center gap-2">
            <button onClick={() => setCollapsedIds(new Set())} className="text-xs text-blue-600 hover:underline">Expand All</button>
            <button onClick={() => {
              const allIds = new Set<string>();
              const collect = (n: OrgNode) => { allIds.add(n.id); (n.children || []).forEach(collect); };
              if (tree) collect(tree);
              setCollapsedIds(allIds);
            }} className="text-xs text-blue-600 hover:underline">Collapse All</button>
          </div>
        )}
      </div>
