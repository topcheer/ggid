...
export default function OrgChartPage() {
  ...
  useEffect(() => {
    setError(null);
      fetch("/api/v1/orgs", { headers: { ...authHeader(), "X-Tenant-ID": localStorage.getItem("ggid_tenant_id") || "" } }).then(async (res) => {
  ...
  const fetchTree = useCallback(async (orgId: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/orgs/${orgId}/tree`, { headers: { ...authHeader(), "X-Tenant-ID": localStorage.getItem("ggid_tenant_id") || "" } });
