  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={startNew} className="flex items-center gap-1 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500"><Plus className="h-3 w-3" /> {t("flags.create")}</button>
      </div>
      ...
      {tab === "list" && (
        <div className="space-y-3">{flags.length === 0 ? (
          ...
          <div key={f.key} className={`${card} flex items-center justify-between !p-3`}>
            ...
            <div className="flex items-center gap-2"><button onClick={() => startEdit(f)} aria-label={"Edit " + f.key} className="rounded p-1.5 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"><Edit className="h-3.5 w-3.5" /></button><button onClick={() => toggleFlag(f.key)} aria-pressed={f.enabled} className={`relative h-6 w-11 rounded-full transition ${f.enabled ? "bg-green-500" : "bg-gray-300 dark:bg-gray-700"}`}><span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition ${f.enabled ? "left-5" : "left-0.5"}`} /></button></div>
          </div>
        ))}</div>
      )}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className={card}>
            ...
            <div className="space-y-3">
              ...
              <button onClick={saveFlag} disabled={!fName || !fKey || saving} className="flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} {t("flags.save")}</button>
            </div>
          </div>
          <div className={card}>
