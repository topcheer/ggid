...
Updated 2026-07-22 after auth pod restart race condition reset it.
...
规则：禁止用 gen_hash.go 手动重置 admin 密码。auth pod 重启时的 cred sync race condition 可能导致密码被重置回 bootstrap 默认值(无尾A版本)。
