...
func (r *quotaRepo) CheckQuota(ctx context.Context, tenantID, metric string) (bool, int, error) {
	...
	case "api_key_count":
		...
		current = usage.APIKeyCount
	case "session_count":
		limit = quota.MaxSessions
		current = usage.SessionCount
	default:
...
func (h *HTTPHandler) handleTenantQuota(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut, http.MethodPost:
		...
		if req.Plan != "" {
			...
			q.MaxUsers = req.MaxUsers
			q.MaxAPIKeys = req.MaxAPIKeys
			q.MaxSessions = req.MaxSessions
			q.MaxStorageMB = req.MaxStorageMB
