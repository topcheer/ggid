...
func TestSecretsProvider_Env(t *testing.T) {
	t.Setenv("GGID_TEST_VAR", "test-secret-value")
	provider := NewSecretsProvider()
	val, err := provider.GetSecret(context.Background(), "env://GGID_TEST_VAR")
	if err != nil {
		t.Fatalf("env resolution should not error: %v", err)
	}
	if val != "test-secret-value" {
		t.Errorf("GGID_TEST_VAR should be test-secret-value, got %q", val)
	}
}

func TestSecretsProvider_EnvBlocked(t *testing.T) {
	provider := NewSecretsProvider()
	// Non-GGID_ env vars must be blocked to prevent reading
	// sensitive variables like DATABASE_URL or API keys.
	_, err := provider.GetSecret(context.Background(), "env://PATH")
	if err == nil {
		t.Error("non-GGID_ env var should be blocked")
	}
}
...
func TestSecretsProvider_PlainValue(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("plain value should not error: %v", err)
	}
...
func TestSecretsProvider_VaultNotConfigured(t *testing.T) {
	...
	if err == nil {
		t.Error("vault without config should error")
	}
}
