...
func TestBackupRepo_TriggerNilPool(t *testing.T) {
	repo := newBackupRepo(nil)
	backup, err := repo.TriggerBackup(nil, "test-tenant", "full")
	if err != nil {
		t.Fatalf("nil pool should not error: %v", err)
	}
	if backup.Type != "full" {
...
func TestBackupRepo_VerifyNilPool(t *testing.T) {
	repo := newBackupRepo(nil)
	if err := repo.MarkVerified(nil, "test-tenant", "test-id"); err != nil {
...
func TestBackupRepo_CompleteNilPool(t *testing.T) {
	repo := newBackupRepo(nil)
	if err := repo.MarkCompleted(nil, "test-tenant", "test-id", 2048, "s3://test"); err != nil {
		t.Errorf("nil pool should not error: %v", err)
	}
}
