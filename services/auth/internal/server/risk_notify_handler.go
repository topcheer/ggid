...
func (h *Handler) handleRiskNotify(w http.ResponseWriter, r *http.Request) {
	...
	if req.UserID == "" || req.EventType == "" {
		writeError(w, http.StatusBadRequest, "user_id and event_type required")
		return
	}
	channel := req.Channel
	if channel == "" {
	...
	case "new_device":
		severity = "medium"
		message = "New device sign-in detected"
	case "impossible_travel":
		severity = "high"
		message = "Impossible travel detected — possible account compromise"
	case "brute_force":
		severity = "critical"
