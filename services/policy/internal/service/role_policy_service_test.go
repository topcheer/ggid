...
func TestRoleService_ListRoles(t *testing.T) {
	...
	tid := uuid.New()
	for i := 0; i < 3; i++ {
		svc.CreateRole(context.Background(), tid, "r", "R", "", nil)
	}
...
func TestRoleService_DeleteRole_SystemRole(t *testing.T) {
	...
	repo.roles[sr.ID] = sr
	svc := NewRoleService(repo, &mockPermRepo{}, &mockUserRoleRepo{})
	if err := svc.DeleteRole(context.Background(), sr.ID); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestRoleService_DeleteRole_NotFound(t *testing.T) {
	svc := NewRoleService(&mockRoleRepo{}, &mockPermRepo{}, &mockUserRoleRepo{})
	if err := svc.DeleteRole(context.Background(), uuid.New()); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestRoleService_ListPermissions(t *testing.T) {
	...
	tid := uuid.New()
	for i := 0; i < 3; i++ {
		svc.CreatePermission(context.Background(), &domain.Permission{TenantID: tid, Key: "k"})
	}
...
func TestPolicyService_ListPolicies(t *testing.T) {
	...
	tid := uuid.New()
	for i := 0; i < 3; i++ {
		svc.CreatePolicy(context.Background(), &domain.Policy{TenantID: tid, Effect: domain.EffectAllow})
	}
...
func TestGetEffectivePermissions_WithChildren(t *testing.T) {
	...
	// Parent should have 1 (its own only — no ancestors)
	perms, err := svc.GetEffectivePermissions(context.Background(), parentID)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
	if len(perms) != 1 {
		t.Fatalf("expected 1 perm for parent (own only), got %d", len(perms))
	}

	// Child should have 2 (its own + inherited from parent)
	perms, err = svc.GetEffectivePermissions(context.Background(), childID)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
	if len(perms) != 2 {
		t.Fatalf("expected 2 perms for child (own + parent), got %d", len(perms))
	}

	// Grandchild should have 3 (its own + child + parent via ancestor chain)
	perms, err = svc.GetEffectivePermissions(context.Background(), grandchildID)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
	if len(perms) != 3 {
		t.Fatalf("expected 3 perms for grandchild (own + ancestors), got %d", len(perms))
	}
}
...
func TestGetEffectivePermissions_MultipleChildren(t *testing.T) {
	...
	// Parent has no own perms and no ancestors = 0 perms
	perms, err := svc.GetEffectivePermissions(context.Background(), parentID)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
	if len(perms) != 0 {
		t.Fatalf("expected 0 perms for parent (no own perms, no ancestors), got %d", len(perms))
	}

	// Child1 should have only its own perm (parent has none to inherit)
	perms, err = svc.GetEffectivePermissions(context.Background(), child1ID)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
	if len(perms) != 1 {
		t.Fatalf("expected 1 perm for child1 (own only), got %d", len(perms))
	}
}
