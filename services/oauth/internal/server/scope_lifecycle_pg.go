...
func (s *pgScopeLifecycleStore) Get(ctx context.Context, scopeID string) (*ScopeLifecycle, bool) {
	var sl ScopeLifecycle
	var approverBytes []byte
	...
	if len(approverBytes) > 0 {
		json.Unmarshal(approverBytes, &sl.ApproverChain)
	}
	return &sl, true
}
...
func (s *pgScopeLifecycleStore) List(ctx context.Context) ([]*ScopeLifecycle, error) {
	...
	for rows.Next() {
		var sl ScopeLifecycle
		var approverBytes []byte
		if err := rows.Scan(&sl.ScopeID, &sl.RequestedScope, &sl.Requester, &approverBytes, &sl.Status, &sl.RiskLevel, &sl.AutoExpireDays, &sl.CreatedAt); err != nil {
			return nil, err
		}
		if len(approverBytes) > 0 {
			json.Unmarshal(approverBytes, &sl.ApproverChain)
...
func (a *scopeLifecycleAdapter) Put(sl *ScopeLifecycle) {
	if a.pg != nil {
		a.pg.Put(context.Background(), sl)
		return
	}
...
func (a *scopeLifecycleAdapter) Get(scopeID string) (*ScopeLifecycle, bool) {
	if a.pg != nil {
		sl, ok := a.pg.Get(context.Background(), scopeID)
		if ok {
			return sl, true
		}
	}
...
func (a *scopeLifecycleAdapter) List() []*ScopeLifecycle {
	if a.pg != nil {
		list, _ := a.pg.List(context.Background())
		if list != nil {
			return list
		}
	}
