...
func (s *pgReviewStore) Get(ctx context.Context, id string) (*AgentReview, bool) {
	...
	if len(scopesBytes) > 0 {
		json.Unmarshal(scopesBytes, &rv.ScopesReviewed)
	}
	return &rv, true
}
...
func (s *pgReviewStore) ListByAgent(ctx context.Context, agentID string) ([]*AgentReview, error) {
	...
	for rows.Next() {
		var rv AgentReview
		var scopesBytes []byte
		if err := rows.Scan(&rv.ID, &rv.AgentID, &rv.Reviewer, &scopesBytes, &rv.Decision, &rv.Comment, &rv.Timestamp); err != nil {
			return nil, err
		}
		if len(scopesBytes) > 0 {
			json.Unmarshal(scopesBytes, &rv.ScopesReviewed)
...
func (a *reviewAdapter) Put(rv *AgentReview) {
	if a.pg != nil {
		a.pg.Put(context.Background(), rv)
		return
	}
	a.mu.Lock()
	a.store[rv.ID] = rv
...
func (a *reviewAdapter) Get(id string) (*AgentReview, bool) {
	if a.pg != nil {
		rv, ok := a.pg.Get(context.Background(), id)
		if ok {
			return rv, true
		}
	}
...
func (a *reviewAdapter) ListByAgent(agentID string) []*AgentReview {
	if a.pg != nil {
		list, _ := a.pg.ListByAgent(context.Background(), agentID)
		if list != nil {
			return list
		}
	}
